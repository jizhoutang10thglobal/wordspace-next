...
| 2 | §5.1 用户时刻 + 性能预算（≤100ms / 内存≤20MB） | 三件可见实物（PR / `npm test` 绿 / macOS 能用）+ compound 实物。headless 量不到真窗口性能，性能/手感留人肉眼。 |
| 3 | §5.3 五维非功能验收（Scale/Time/Cross-OS/Concurrency/Failure） | 跨平台/长时间/大规模/并发砍进 §2 Out；客观项落到 §5.2 Vitest 或 §5.3 无 DISPLAY 自动 skip 的 E2E。 |
| 4 | §5 单一验收层 | 拆成 §5.2 Vitest 必过（权威绿/红门，纯逻辑）/ §5.3 Electron E2E（容器 skip，人在 macOS 跑）。 |
