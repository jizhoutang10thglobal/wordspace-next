...
test('mkWebId：带时间戳,跨会话 seq 复用也不撞键', () => {
  const a = T.mkWebId(1, 1751700000000);
  const b = T.mkWebId(1, 1751700000001); // 同 seq、不同时刻（模拟重启后 seq 从 1 重数）
  assert.ok(T.isWebKey(a) && T.isWebKey(b));
  assert.notEqual(a, b);
});

test('isWebKey / isWebEntry 识别', () => {
  assert.ok(T.isWebKey('web:3:abc'));
  assert.ok(!T.isWebKey('a.html'));
  assert.ok(!T.isWebKey('/abs/x.html'));
  assert.ok(T.isWebEntry(web('web:1:x', 'https://a.com')));
  assert.ok(!T.isWebEntry(f('a.html')));
});

test('openEntry：web 标签建条目携带 url,再开保留 url', () => {
  let s = T.openEntry(empty(), web('web:1:x', 'https://a.com', 'A'));
  const e = s.entries[0];
  assert.equal(e.url, 'https://a.com');
  assert.equal(s.activeRel, 'web:1:x');
  s = T.openEntry(s, web('web:1:x', 'https://a.com', 'A')); // 再激活
  assert.equal(s.entries.length, 1);
  assert.equal(s.entries[0].url, 'https://a.com');
});

test('新标签页 url=null 合法建条目', () => {
  const s = T.openEntry(empty(), web('web:1:x', null));
  assert.equal(s.entries[0].url, null);
  invariant(s);
});

test('updateEntry：导航后刷新 url/title,不动 open/pinned/激活', () => {
...
test('pinEntry：钉 web 标签=书签,closeEntry 关标签后书签留存', () => {
  let s = T.openEntry(empty(), web('web:1:x', 'https://a.com'));
  s = T.pinEntry(s, web('web:1:x', 'https://a.com'));
  assert.ok(s.entries[0].pinned);
  s = T.closeEntry(s, 'web:1:x'); // 关标签
  assert.equal(s.entries.length, 1); // 书签还在
  assert.ok(s.entries[0].pinned && !s.entries[0].open);
  invariant(s);
});

test('removeEntry：置顶区 × 删整条书签', () => {
  let s = T.pinEntry(empty(), web('web:1:x', 'https://a.com'));
  s = T.removeEntry(s, 'web:1:x');
  assert.equal(s.entries.length, 0);
});

test('resolveActive：关掉激活 web 标签→回落相邻', () => {
  let s = T.openEntry(empty(), f('doc.html'));
  s = T.openEntry(s, web('web:1:x', 'https://a.com')); // active=web
  s = T.closeEntry(s, 'web:1:x');
  assert.equal(s.activeRel, 'doc.html'); // 回落到 doc
  invariant(s);
});

test('dropEntry：web 标签跨区拖拽（标签↔置顶）', () => {
  let s = T.openEntry(empty(), web('web:1:x', 'https://a.com'));
  s = T.dropEntry(s, 'web:1:x', true, 0); // 拖进置顶区=钉
  assert.ok(s.entries[0].pinned);
  s = T.dropEntry(s, 'web:1:x', false, 0); // 拖回标签区
  assert.ok(!s.entries[0].pinned && s.entries[0].open);
  invariant(s);
});

test('displayOrder：doc + web 混排稳定（置顶在上,标签在下）', () => {
  let s = T.openEntry(empty(), f('doc.html'));
  s = T.openEntry(s, web('web:1:x', 'https://a.com'));
  s = T.pinEntry(s, web('web:1:x', 'https://a.com'));
  const order = T.displayOrder(s.entries).map((e) => T.keyOf(e));
  assert.deepEqual(order, ['web:1:x', 'doc.html']); // 置顶 web 在上,doc 标签在下
});

test('reconcileTree 不动 web 条目（无 rel）', () => {
  let s = T.openEntry(empty(), web('web:1:x', 'https://a.com'));
  const r = T.reconcileTree(s, new Set(['whatever.html']), new Map());
  assert.equal(r.entries.length, 1);
  assert.equal(r.entries[0].abs, 'web:1:x');
});
