...
test('P2-4 含冒号的日常词组落搜索,不误当危险协议拦截', () => {
  // 名单外的 scheme 名 → 搜索（不是网址,spec §5：非网址即搜索）
  const note = U.parse('note:hello');
  assert.equal(note.kind, 'search');
  assert.ok(note.url.indexOf('bing.com') !== -1 && note.url.indexOf('note') !== -1);
  assert.equal(U.parse('todo:fix').kind, 'search');
  assert.equal(U.parse('re:报价').kind, 'search');
  assert.equal(U.parse('myapp:token').kind, 'search'); // 自定义 scheme 从 blocked 变搜索（无害,可接受）
  // 名单内危险协议仍 blocked
  assert.equal(U.parse('javascript:alert(1)').kind, 'blocked');
  assert.equal(U.parse('data:text/html,x').kind, 'blocked');
  assert.equal(U.parse('file:///etc').kind, 'blocked');
  assert.equal(U.parse('mailto:a@b.com').kind, 'blocked'); // v1 不外呼,维持 blocked
  // 端口写法不受影响（冒号后数字不进 scheme 判定）
  assert.deepEqual(U.parse('localhost:8080'), { kind: 'url', url: 'http://localhost:8080' });
});
