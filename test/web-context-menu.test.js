// 网页右键菜单纯逻辑 builder 单测（spec §4.7 六分节；save-link/save-image 走下载管线 §4.11，2026-07-17 恢复；砍除项 §12：无 clip-page 剪藏）。
...
test('http 链接 → 链接节四条在最前（含 save-link 另存为）；危险 scheme 链接整节消失', () => {
  const tpl = M.buildCtxTemplate({ linkURL: 'https://a.com/p?utm_source=x&id=1' }, {});
  assert.deepStrictEqual(ids(tpl).slice(0, 4), ['open-link', 'open-link-bg', 'copy-link', 'save-link']);
  assert.strictEqual(byId(tpl, 'copy-link').args.url, 'https://a.com/p?utm_source=x&id=1');
  assert.strictEqual(byId(tpl, 'save-link').args.url, 'https://a.com/p?utm_source=x&id=1'); // 下载管线拿这个 url
  for (const bad of ['javascript:alert(1)', 'file:///etc/passwd', 'data:text/html,x', 'about:blank']) {
    ...
    assert.strictEqual(byId(t, 'save-link'), undefined, bad); // 危险 scheme 整节消失,save-link 也不出
  }
});
...
test('图片：http srcURL → 拷贝图片+拷贝地址+存储图片；data: srcURL 只留「拷贝图片」（地址类与 save-image 都不出）', () => {
  ...
  assert.ok(byId(http, 'save-image'), 'http 图片有存储图片（下载恢复 §4.11）');
  assert.strictEqual(byId(http, 'save-image').args.url, 'https://a.com/pic.png'); // 下载管线拿 srcURL
  ...
  assert.strictEqual(byId(data, 'save-image'), undefined); // data: 非 http(s) → save-image 不出（安全红线）
});
...
test('全六节同时出现 → 完整顺序 + 5 条分隔符 + 不变式 + 非导航项不被禁用 + 无砍除项', () => {
  ...
  assert.deepStrictEqual(ids(tpl), [
    'open-link', 'open-link-bg', 'copy-link', 'save-link',
  ...
  assert.ok(sepInvariantOk(tpl));
  // save-link/save-image 已恢复（§4.11）；剪藏 clip-page 仍砍（§12）
  assert.ok(byId(tpl, 'save-image'));
  assert.ok(byId(tpl, 'save-link'));
  assert.strictEqual(byId(tpl, 'clip-page'), undefined);
  // 非导航项必须不带 enabled:false（映射层是 enabled!==false 才可点；防误加 enabled:false 让条目变灰失效）
  for (const id of ['reload', 'copy-page-url', 'export-pdf', 'open-link', 'copy-image', 'copy-selection', 'cut', 'save-link', 'save-image']) {
