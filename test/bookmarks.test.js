...
test('P3-09 手动建文件夹去重：连建同名 → 加后缀，与导入一致', () => {
  let s = B.emptyState();
  s = B.addFolder(s, '新文件夹', T0).state;
  s = B.addFolder(s, '新文件夹', T0).state;
  s = B.addFolder(s, '新文件夹', T0).state;
  const names = s.folders.map((f) => f.name);
  assert.ok(names.includes('新文件夹'));
  assert.ok(names.includes('新文件夹 2'));
  assert.ok(names.includes('新文件夹 3'));
});

test('P3-09 rename 撞车加后缀，改成自己现名不误加后缀（排除自己）', () => {
  let s = B.emptyState();
  const a = B.addFolder(s, 'A', T0); s = a.state;
  const b = B.addFolder(s, 'B', T0); s = b.state;
  // 把 B 改名成已存在的 A → 'A 2'
  s = B.renameFolder(s, b.id, 'A');
  assert.strictEqual(s.folders.find((f) => f.id === b.id).name, 'A 2');
  // 把 A 改成它自己的名字 A → 不该变 'A 2'
  s = B.renameFolder(s, a.id, 'A');
  assert.strictEqual(s.folders.find((f) => f.id === a.id).name, 'A');
});
