...
// U2 / select-1：Blink 原生删除/typing-style 机制会往块元素上留**空** style=""（app 从不写），
// 且不带 data-ws-pushed（不是分页来源）→ cleanRoot 必须通用剥空 style，否则入盘即中 block-style 规则、
// 整篇非合规、重开永久降级基础编辑。**非空** style 照旧保留（不误剥、不削弱校验器）。
test('U2: cleanRoot 通用剥空 style=""（非 data-ws-pushed 来源）、保留非空 style', () => {
  const dom = new JSDOM('<!DOCTYPE html><html><body>' +
    '<p style="">选删后原生残留的空 style</p>' +
    '<ul class="ws-todo"><li data-checked="true" style="">乙乙</li></ul>' +
    '<p style="color:red">用户真实内联样式</p>' +
    '</body></html>');
  const out = serializeDocument(dom.window.document);
  assert.ok(!/style=""/.test(out), '空 style="" 必须剥净（漏一个 = 块级 style = 非合规 = 整篇降级）');
  assert.ok(out.includes('选删后原生残留的空 style'), '内容不丢');
  assert.ok(out.includes('data-checked="true"'), '勾选态不误剥（只剥空 style）');
  assert.ok(out.includes('style="color:red"'), '非空 style 必须原样保留（不误剥、不削弱校验器）');
});
