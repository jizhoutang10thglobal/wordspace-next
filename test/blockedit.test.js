...
// docHasAuthorStyles：决定要不要套编辑器 Notion 排版的开关——文档自带样式就不套（保真）。
function docOf(headHtml, bodyHtml) {
  return new JSDOM('<!DOCTYPE html><html><head>' + (headHtml || '') + '</head><body>'
    + (bodyHtml || '') + '</body></html>').window.document;
}
test('docHasAuthorStyles: 真·裸文档（无 style/link）= false → 该套排版', () => {
  assert.equal(be.docHasAuthorStyles(docOf('', '<h1>t</h1><p>x</p>')), false);
});
test('docHasAuthorStyles: 自带 <style> = true → 不套排版（哪怕裸挂 body）', () => {
  assert.equal(be.docHasAuthorStyles(docOf('<style>h1{color:red}</style>', '<h1>t</h1>')), true);
});
test('docHasAuthorStyles: 自带 <link rel=stylesheet> = true', () => {
  assert.equal(be.docHasAuthorStyles(docOf('<link rel="stylesheet" href="x.css">', '<p>x</p>')), true);
  assert.equal(be.docHasAuthorStyles(docOf('<link rel="StyleSheet" href="x.css">', '<p>x</p>')), true); // rel 大小写不敏感
});
test('docHasAuthorStyles: 只有编辑器自己注入的 <style data-ws2-ui> 不算自带样式', () => {
  assert.equal(be.docHasAuthorStyles(docOf('<style data-ws2-ui>p{}</style>', '<p>x</p>')), false);
});
