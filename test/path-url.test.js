...
test('MP-12 htmlPathFromArgv：以 - 开头的 flag（值以 .html 结尾）不被当成文件', () => {
  assert.equal(htmlPathFromArgv(['app.exe', '--user-data-dir=/x/y.html', '/abs/real.html'], '/cwd'), '/abs/real.html');
  assert.equal(htmlPathFromArgv(['app.exe', '--foo=/tmp/a.md'], '/cwd'), null);
});
