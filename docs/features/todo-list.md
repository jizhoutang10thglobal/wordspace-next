...
一会儿在框内一会儿在框外（实测行选框左缘 306.2、整表框左缘 279，差 27.2）。修法：`li` 盒向左扩到与 ul 齐平
（`margin-left:-1.7em`）再用等量 `padding` 把内容推回原位——**文字与勾选框的绝对位置一字不动**，只是边框盒
变宽把勾选框收了进来；用 `em` 跟着 `:where(ul,ol)` 的 `1.7em` 走，嵌套缩进由每层 ul 自己的 `padding-left` 给、
逐层对冲后一层都不塌。⚠ 连带拔掉一个脆弱契约：`todoGutterHit` 的 X 命中带原写死成 `[li.left-26, li.left-2]`
