...
**「上一块是列表」的合并形态。** 段落行首退格并入上一个列表时，并进**末项的文字**拼成一项，**不是追加成新列表项**（Notion 实测：`父后行` + `分隔二` → 一项 `父后行分隔二`）。这也是列表行剥离后再退一次的落点，两条路径必须同款。⚠ 断言这条时**只看块级 textContent 是哑门**——「一项『甲乙』」和「两项『甲』『乙』」的块文字长得一模一样（变异自检实测：改回追加成新项，只看块文字的断言照样全绿），必须逐项查 `li`。
...
- ~~**多级裸嵌套 marker 层级不区分**（PR-C U11）~~ → **2026-08-04 已解**：基线语义 CSS 加了逐级循环（`ol ol`→lower-alpha、`ol ol ol`→lower-roman、`ul ul`→circle、`ul ul ul`→square，第四级回首档），对齐 Notion 实测的 1./a./i. 与 •/◦/▪。**todo 内部的裸嵌套列表维持 disc/decimal 不变**（`.ws-todo ul:not(.ws-todo)` 类选择器压过零权重 `:where()`，PR-C U11 的设计意图保留）。门：`e2e/list-nested-markers.spec.js`。
