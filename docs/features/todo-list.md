...
**gutter「+」快捷插入钮（粒度对齐 U4=D，分支 feat/ux-granularity，2026-08-03）。** ⋮⋮ 手柄左侧一格常驻「+」，**与手柄同显同隐**（显隐统一走 `setGutterVisible`——分开控制必漏一处、产幽灵按钮）、与手柄同一水平线、`cursor:pointer`（与手柄的 `grab` 区分）。**作用对象与手柄同口径**：行锚（悬停行）= 在同一张列表插入新的空行（嵌套行插进同一层子列表）；块（非列表块，或 Esc 灰选整块）= 插入空正文块。**⌥ 点击插到上方**（对齐 Notion「Click to add below · Option-click to add above」）。插入后光标落新行/新块，一次 undo 整体还原；`title` 走 i18n（`editor.plusTip`，zh/en 双词条）。门：`e2e/list-row-plus.spec.js`（8 条）。
