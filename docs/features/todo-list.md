...
**勾选。** 点勾选框只切 `data-checked`、不进编辑态、不放光标（待 PR-B）。勾选视觉：文字变灰 + 划线、框变蓝底白✓；取消勾选完全还原。勾选态落盘、关开重开保留、不触发降级。
...
**（待 PR-B~E 移入契约）** Enter 分裂不克隆 id/勾选态；块末 Delete 与 Backspace 对称；Tab/Shift-Tab 嵌套与顺序；多项列表↔文本转换往返；嵌套子列表渲染；勾选热区几何、对比度、深色 emoji。
...
- app 侧：commit `d95f4c5`（2026-07-23，PR-A 前的最新 main）+ 本 PR（PR-A：U1/U2/U3）

## 欠账

- **ui-demo 侧 todo 行为未审计**——本 spec 行为契约先按真 app 定稿，ui-demo 是否一致待专门对齐（记账不阻塞 PR-A）。
- **行为契约只落地了 PR-A 三条**（创建补 `<br>`、编辑流不留空 style、剪贴板块级打包）；勾选/键盘流/转换/嵌套/视觉等契约随 PR-B~E 逐批移入，每批同 PR 更新本段与对齐锚点。
