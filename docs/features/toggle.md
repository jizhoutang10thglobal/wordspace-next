...
- **跨界删除契约（U26，Colin 2026-07-24「块操作与其他块同步」）**：①同一 toggle 内跨 summary↔正文的选区删除——全覆盖（summary 头→末正文块尾）=**整删 toggle**；部分覆盖=裁剪式（summary 裁尾、中间正文块删、末块裁头，**绝不合并** summary 与正文，正文删空补 `<p>`）。②选区跨 toggle 外边界（一端在内一端在外/跨两个 toggle）=**端点上卷、toggle 整删**（与块级高亮 refreshRangeSel 同款上卷=所见即所删；对齐 table 的 ED-A2 结构端点整块删）。③cut/打字覆盖同契约（三路都走 deleteSelection）；summary 编辑态的跨界选区不交原生（原生半删）。旧「跨界一致化空操作 + flashNope」（a254cb6，deferred）随本契约废除。门：`e2e/block-range-select.spec.js` U26a-e + `toggle.spec.js` BF-P2(U26)。
