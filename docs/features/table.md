...
- **矩形选区与表界钳制**（2026-08-06，对拍 T13/T14 复拍实测，PR-5；Notion 原始读数 = 复拍报告
  ...
  - **行拖拽（T3/T4）实测成立、实现押后（PR-5c 欠账）**：Notion 行药丸（`.notion-simple-table-selector`）
    单击=开行菜单，**按住拖=移动整行**（t3f 修正读数：行 2/行 3 互换成功；拖拽中被拖行渲染为横跨
    表宽的幽灵行、落在行槽间），块手柄拖=整表移动。⚠ 教训入账：早先四次「行不可拖」阴性全是
    mousePressed 缺 buttons:1 的探针假象——**阴性结论必须有同通道正对照**才可信。
