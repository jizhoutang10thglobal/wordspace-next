...
- **矩形选区与表界钳制**（2026-08-06，对拍 T13/T14 复拍实测，PR-5；Notion 原始读数 = 复拍报告
  ...
  - **出向钳制**：拖出表界，指针坐标四向夹回表内（`cellPosAtPoint`），矩形恒在表内（Notion f3/f3b
    上下对称实测）。**入向钳制**：表外起的选区端点伸进表内 → 端点截到表界（锚进相邻块**内部**——
    body 层锚点会让删除管线判块外死键）。**有意分歧记录**：Notion 实测入向是「整个不产生选区」
    （f4/f4b 两次实测，按点在文字头/尾结论一致）；我们保留段落侧文字选择——对用户更有用，且两者
    共同守住「表格绝不被部分圈选」的硬边界。旧「端点在表内=整表蓝+整删」通道退役。
  - **边缘入口 T2**：贴近表**下缘**悬停出全宽「+」横条（点击=末行下插行）、贴近**右缘**出全高
    「+」竖条（点击=末列右插列），几何判定不依赖悬停命中表块（Notion t2b 实测：条宽=表宽/条高=
    表高，点击 rows 3→4 / cols 3→4）。
  - **行拖拽（T3/T4）复拍推翻，不做**：Notion 行药丸（`.notion-simple-table-selector`）单击=开行
    菜单/选行，**按住拖=扩格选区而非移动行**（四次独立实测：药丸拖/先选再拖/块手柄拖/直接拖），
    块手柄拖=整表移动。plan 里「行手柄可拖+行间指示线」的旧口径系第二批误读，账在此清。
