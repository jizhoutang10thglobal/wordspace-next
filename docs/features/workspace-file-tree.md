...
- **右键菜单视口感知定位 + 滚动即关**（Wendi 2026-07-21 视频反馈，`showContextMenu`）：菜单默认左上角贴点击点，
  但**下方放不下就翻到点击点上方**（菜单底边落在点击点）、**右侧放不下就翻左**，再夹进视口留 6px 边距——
  修「右键靠底部的文件时菜单往下展开、被窗口底边裁掉、下半截看不见」（实测 720 高视口下溢出 88px）。
  尺寸用 `offsetWidth/Height`（布局尺寸，不受 `ws-pop-in` 入场动画的 scale/translate 干扰；`getBoundingClientRect`
  会把动画瞬态算进去，不能拿来定位）。另外菜单是 `position:fixed`、不跟 `#sb-body` 内容滚动——**开着菜单滚侧栏 /
  缩窗口即关菜单**（`ctxDismiss` 挂 `#sb-body` scroll + window resize，关菜单时拆监听），免得菜单飘在原地、脱离
  右键的那一行。
...
- app 侧：分支 `fix/ctx-menu-position`（2026-07-21，右键菜单视口感知定位（下溢翻上/右溢翻左/夹进视口）+ 滚动缩窗即关，Wendi 视频反馈）
