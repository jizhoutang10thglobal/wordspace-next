...
状态（2026-07-14）：**设计契约，两侧均未实现**。由插图技术研究落账，待 Colin/Wendi 拍
「待拍板」三项后按「ui-demo 先定稿 → 移植真 app」流程实施。
...
1. **粘贴截图/图片**（主路径）：剪贴板含 `image/*` → 降采样管线 → 在光标所在块之后插入图片块。
...
1. 剪贴板同时含文本和图片时谁优先（Notion 行为 = 图片优先）。
2. 光标在空段落上插入图片：块后新插 vs 原地替换空段落。
3. Phase 2 的触发时机与 sidecar 命名（`<文档名>.assets/` vs 共享 `assets/`）。
