...
| 单元 | 内容 | 规格 |
|---|---|---|
| **U4** 消费面 | 悬停预览卡 + 断链装饰（CSS Highlight）+ 断链修复卡 | §5.1–5.3 |
| **U5** 改名/移动重写 | 三挂钩（app 内改名/移动 + 外部改名）→ 自动重写引用 + 撤销 | §5.4 |
| **U6** 反链面板 + 删除守卫 | 「N 篇文档链接到这里」+ 删除被引用文件的警告 | §5.5–5.6 |
| **U7** doc-id 修复锚 | 保存时补 `<meta name="wordspace-doc-id">`；修复卡候选升级 | §6.4 |
| **U8** e2e 强门收口 | 全表覆盖 + 变异自检 | §7 |
...
| `http:` / `https:` | ipc `open-external-url`（scheme 白名单）→ 系统浏览器 |
| `#锚点` | iframe 内 scrollIntoView，不导航 |
| `mailto:` / `javascript:` / 其它 scheme / 越根 / `\\` UNC | preventDefault 拦下不动作 |
...
验收中。真 app 移植设计（基于代码勘察，seam 都核实过）：
...
- **UI 落点**：真 app 没有 ui-demo 那种侧栏顶（红绿灯+地址栏）。按钮放**文档头 `#doc-header`
  面包屑左侧**（`.ws-breadcrumb` 前面，那里现在是空的）：两颗 chevron 图标钮（‹ ›），
...
- **开放口径（Wendi 验收中，实现前确认）**：历史算不算「切标签」？当前 ui-demo 实现 = 点树/点链接/
  切标签**都**入历史（"back = 上一篇看过的"）。另一口径 = 只有链接跳转入历史。**默认按前者移植**，
  Wendi 有裁决再调（模型上只是「push 挂在 openDoc 还是只挂 onDocLinkClick 路径」的开关）。
...
- **候选「重新指向」**：最多 **3 条**，每条「重新指向 {候选路径}」。候选 = 同根内**纯文件名精确
  相等**（`baseOf` 相等）的文件（U7 落地后升级为：doc-id 全库匹配 > 同名 > ino 历史，优先级递减）。
- **恒有一条「新建」**：「在 {目标目录 或 '根目录'} 新建「{文件名}」」——**尊重断链的扩展名**
  （断链指向 `.md` 就建 `.md`，否则 `.html`）。
...
**且**旧路径未被占；不满足 → toast「**文件已被后续操作改动，无法撤销这次链接更新**」（neutral），
明说放弃、不做半套。
...
`dirOf / baseOf / normalizePath / splitHrefSuffix(href) → {path, suffix} / resolveHref(fromRel, href) → rel|null /
...
realpath（父目录 realpath + basename，容忍目标不存在）→ `ownerOf` 判根内 → {kind, abs, rel, rootId, name}
| {miss} | {outside}。拒 file: URL 带 host / 非 file scheme。
