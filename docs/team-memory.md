...
## 2026-07-14 — 探索测试找出 1 P1+5 P2+11 P3;修复计划 18 条已成册(PR #195),待 Colin 拍板后认领

**是什么**:4 路 AI 探索测试(Playwright 驱动真 app 当用户,每 bug 复现 ≥2 次)扫了浏览器/收藏历史/文件管理/多根+视觉,P1 与全部 P2 经源码核实成立且带 file:line 根因。头部发现:错误页后再导航 UI 永远停在错误占位(P1,browser.js showError 摘 view 后清占位分支永不命中)/文件夹在 app 内完全无法移动(前端没接 draggable,后端守卫是死代码)/连删只能撤销最后一个/omnibox 打字中切标签残留误导航/`note:hello` 被 OPAQUE_SCHEME_RE 误拦/吸顶克隆行漏 ondragover+ondrop 成拖放死区。干净区:收藏历史全链路、dirty 文档改名/移动零丢数据。

**怎么 apply**:①修复计划一条一个文件在 `docs/plans/bug-hunt-2026-07-14/`(PR #195,**不 auto-merge,等 Colin 拍板**),每条含复现/根因/修法/门(变异自检)/spec 记账,README 有执行纪律——**拍板合入后认领执行,一条计划=一个分支=一个 PR**,修完回 team-memory 广播一行;②p2-1 与 p2-5 同区(sidebar.js 拖拽/renderSticky),建议同一 session 接;③两条内含拍板项(p3-02 置顶×钮、p3-10 导入翻倍)没拍前别动;④在此之前**别顺手修**报告里提到的这些 bug(计划里有测试门与 spec 记账要求,散修会漏门)。

**来源**:探索测试试点(Colin 2026-07-14 拍板);汇总 Artifact 在 PR #195 描述里;方法论/作战手册可复用,下轮发版前可再跑。
