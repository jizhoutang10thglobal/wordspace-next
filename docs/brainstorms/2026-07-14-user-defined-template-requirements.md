...
允许用户自定义模板（Wendi 2026-07-14 澄清：本 feature 的对象是 **Template，不是 Schema**）。一个模板 = 一小包数据：**受管制的样式 CSS + 可选起始骨架 + 元数据**。应用模板 = 把样式盖章写进文档 `<head>`（带标记的 `<style>`），文件自包含——发给别人、导 PDF、浏览器直开都好看，对方什么都不用装。支持：从模板新建、给既有文档一键换装/卸装、把当前文档存为模板、AI 按描述生成模板。分享零后端：本地文件导出导入 → 公司库 = 共享云盘文件夹（复用已有多根工作区能力）→ 公共池 = 静态索引一次 GET，全程不需要账号或自建云。
...
- R11.（pending Q3）工作区内的模板目录自动进画廊：公司私有库 = 把共享云盘文件夹加进工作区（复用多根能力）。扫描按模板文件**逐个**过 R2 门：单个违规模板被排除并可查原因、不连坐同文件夹其余模板；共享目录首次出现新模板时给来源提示（哪个文件夹、何时出现），不完全静默生效。
- R12.（pending Q3）公开模板池 = 静态索引拉取（无账号）；拉到的模板与本地模板走完全相同的 R2 门与管理界面。
...
- AE6. **Covers R10, R11（R11 pending Q3）.** Given 同事发来模板文件 / 共享文件夹里有模板，When 导入 / 加入工作区，Then 模板**逐个**过安全门后出现在画廊（违规的单独排除并可查原因、不连坐其余），首次出现给来源提示；全程无任何网络请求或账号操作。
...
- 公开模板池的正式运营（官方索引的策展、更新节奏）——v1 只需机制成立（R12），池子本身可以后铺。
...
- Q1（Wendi）：模板语义确认——纯版式，还是版式 + 可选骨架（本文按后者立义）？「存为模板」默认要不要带内容骨架？
- Q2：既有文档一键换装进不进 v1？（本文强烈推荐进——架构上是干净替换，且是相对 Word/Notion 的差异化点；砍掉它模板就退化成「新建时的皮肤」。）
- Q3：v1 分发档位切到哪——只本地 + 文件导入导出，还是连公司库（共享文件夹，R11）一起上？公共池（R12）v1 做机制还是连最简官方索引一起做？（R11/R12 及 F4/AE6 均已标 pending 本题；`.md` 要不要开模板通道也并入本题一起拍。）
- Q5（Wendi）：品牌字体 `data:` 内嵌与「文件自包含」的体积取舍——中文字体动辄数 MB，R15 的表达力黄金用例直接依赖这个口径（可选方向：字体子集化、只嵌标题字重等，细节归 planning，但「嵌不嵌」要产品拍）。
- Q4：按惯例（大 UX feature 先 ui-demo 定稿再移植真 app），本 feature 是否走 ui-demo-first？画廊/换装预览的交互适合先在 ui-demo 给 Wendi 过目。
