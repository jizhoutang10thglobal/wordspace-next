...
让用户像官方一样定义自己的 Schema。愿景口径（Colin）：**Schema = 一种文档类型**——Word、Excel、PowerPoint、Notion、Photoshop 那样的交互模式与呈现内容，只要原生 HTML 承载得了、在 Wordspace 框架内，都可以成为一个 Schema；官方 Schema 和用户 Schema 是同一种东西。本文回答三个研究问题：① Schema 由什么构成、哪些固定哪些灵活；② 要向用户暴露哪些接口；③ 用户怎么定义（结论：AI 辅助是主路径）。核心研究结论：**呈现与合法性可以完全声明式地开放给用户；交互是编辑器代码，由官方引擎供给（远期插件化）**——用户 Schema = 选一个交互引擎 + 一份声明式定义，校验器、编辑器配置、AI 创作指南三样从同一份定义派生。
...
把 KD2 的三件套和「文档不跑 JS」放在一起推，得出本文最重要的架构判断（**待 Colin/Wendi 正式拍板**，见 Q1）：

> **一个 Schema = 声明式定义（呈现 + 合法性，用户可完全自定义） + 交互引擎绑定（代码，官方供给）。**
...
- **Tier 3 · 引擎级自定义（远期平台玩法）**——让用户/开发者/AI 造新交互范式 = 编辑器插件平台（稳定 SDK、插件安全模型、质量门），或「AI 按描述写引擎代码」。本文不展开，只要求架构不把这条路焊死（registry 的 descriptor 本来就是代码单元，口天然留着）。
...
- Tier 3 插件平台：编辑器 SDK、插件安全模型、让用户/AI 造新交互引擎。架构留口，不设计。
...
- Q1. 核心架构判断要正式拍板：「呈现与合法性声明式开放，交互引擎由官方供给」——它意味着本 feature **不承诺**用户能发明新交互（那是 Tier 3 远期）。Colin 倾向已在对话里表达，但和 Wendi 的愿景对齐要走一次。
