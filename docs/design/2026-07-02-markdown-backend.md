...
**状态**：已实现（2026-07-03，分支 feat/md-backend；plan 见 `docs/plans/2026-07-03-001-feat-markdown-backend-app-plan.md`）
**日期**：2026-07-02
**落地勘误**：本文原判「renderer 零改动」有一个漏洞——渲染是 iframe **`file://` 直载磁盘文件**，
`.md` 直载会被 Chromium 当纯文本渲染。实际落地为「renderer 近零改动」：`.md` 改走既有的
`loadFromHtml`（srcdoc）渲染 read-doc 的转换产物（`<base>` dirUrl 注入现成，相对图片照常解析），
外加 kind `'md'` 三处分流、导出 PDF 强制 wordspace 模式（raw 会直印裸 markdown）、另存为传 ext 保格式。
主进程侧与本文一致（read/save 两端插适配器，`src/main/md-adapter.js`）。ESM 动态 import 在打包
asar 内已实证可用（Electron 42，探针跑真打包 app 主进程转换成功），无需降级备选。
...
## 落地：3 个插点（renderer 近零改动——渲染分支例外，见顶部「落地勘误」）
...
转换全在**主进程**，编辑器 / 校验器一律不动（renderer 有一处渲染分支例外——`.md` 走 srcdoc，见顶部勘误）：
