...
## 与 schema-1 的 `shell.js` 交汇（rebase 钩子）

「非合规不碰」只是文件级隔离；`src/renderer/shell.js` 是本分支与 `feat/schema-1` 唯一都改的文件。
schema-1 session 的 `docs/handoff-schema1-integration.md` 列了四处语义交互，已核对 + 处理：

1. **`save()` 序列化行**：我的 tempDoc 分支在序列化行之前 early-return，不碰那行。rebase 取 schema-1 的
   `basicEdit ? WS2BasicEdit.serialize : WS2Serialize` 三元（临时文档恒合规、用 WS2Serialize 正确）。已加注释。
2. **`renderTemp` 的 `docConform`**：schema-1 的 iframe onload 按 `docConform` 挂块/基础编辑器，`loadFromHtml`
   故意不改它 → 临时文档会读到陈旧判定、编辑器错挂。**rebase 后在 `renderTemp` 调 `loadFromHtml` 前加
   `docConform = routeDoc(rec.html)`**。已在代码里留醒目注释（那两个符号我 base 里没有，现在加不了）。
3. **真收起 reposition（本轮已修的真 bug）**：收起把 iframe 宽 260→0 横移，块编辑器宿主浮层会飘。已加
   `window.__shellReposition`（`blockEdit.reposition()`）+ `setSidebarCollapsed` 里 rAF 调它。rebase 再补
   `basicEdit.reposition()`。
4. **导出 `exportPdf(mode)`**：没碰导出、加 Cmd+P 只在 `onMenu` 加一行没删导出行。rebase 取 schema-1 的
   `exportPdf(basicEdit ? 'raw' : 'wordspace')`。

**合并顺序**：ux-fixes→main → schema-1 rebase 上来 → align 最后叠（由本分支把上面四处一次合掉）。
