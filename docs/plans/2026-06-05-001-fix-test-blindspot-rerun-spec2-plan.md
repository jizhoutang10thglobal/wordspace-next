...
### KTD2. sandbox 修法用方案 C（renderer 直接 require），不用 sandbox:false
方案 C 零成本、保 sandbox、不破解耦（Finding 4）。`sandbox: false`（我在 spec2-preview 工作区做的临时验证）只是用来坐实根因，**不进 main、不作为最终修法**。

### KTD3. 不手动修 app —— 修法靠 CLAUDE.md 教训，由重跑的 AI 自己写对
用户已选「重跑」路线。手动改 app 代码会被重跑覆盖、白改。正确做法：把方案 C 的指导写进 CLAUDE.md 教训，重跑时 AI 读到 → 写对。门是兜底：万一 AI 没照做，CI e2e 真跑会 fail，暴露问题。教训预防 + 门兜底，双保险。
