...
布局、图标、文案、交互时序、快捷键、右键菜单分节与条目、地址栏行为、收藏区的位置与折叠交互、
...
### 4.4 网页态外壳：无网页头（`WebView.tsx`）
> **这是 2026-07-10 的决定**（Wendi + Colin review）：**删掉网页头**（原来网页内容上方那条「锁 + 标题 + 域名」）。
> 冗余——URL 和安全指示地址栏（在左侧栏）已经有了，网页头是重复。
- **长啥样**：网页态**没有任何 Wordspace chrome 罩在网页上**，网页内容直接铺满内容区（mock 站 / 真 iframe）。
  URL / 安全指示 / 收藏全部在**左侧栏**（地址栏 + 收藏区），不在网页上方。
- **真 app**：**不要**在 `WebContentsView` 上方叠 `#web-header` 那一层。安全指示做进左侧栏地址栏
  （读 `webContents.getURL()` 的协议 + 证书状态），不在网页态单独放一条。

### 4.5 收藏区（左侧栏，默认收起）（`ArcSidebar.tsx` `.arc-fav` + `FavChip`）
> **口径反复过，以此为准**：收藏经历了 侧栏平铺区（臃肿）→ 网页态书签栏（2026-07-09）→ **左侧栏折叠收藏区（2026-07-10，最终）**。
> 最终方案：收藏回到**左侧栏**，放在「置顶」**上方**（同 Arc 的 Favorites 在最顶），**默认收起、点标题行才展开**——
> 用折叠解决当初嫌侧栏臃肿的问题。网页态不再有书签栏（连同网页头一起删）。
- **长啥样**：左侧栏最顶一个可折叠区。收起态=一行标题（`⭐ 收藏` + 数量 + 右侧 `Bookmark` 管理入口，管理入口 hover/展开才显）。
  展开态=书签列表：平铺「书签栏」文件夹（`BM_BAR`）的收藏（每项=首字母彩块 + 标题），其余非空文件夹作为分组（文件夹名小标题 + 其下书签）。空态显示提示。
- **交互**：点标题行 toggle 展开/收起（默认收起）。点书签=**新标签打开**该 URL + 记历史。管理入口进 `/bookmarks`。
- **图标细节**：书签没有真 favicon，用**标题首字** + 从 URL hash 出的稳定色当小方块（`FavChip`，比灰地球更好认）。
- **真 app**：收藏区做进左侧栏（置顶上方），默认收起可展开。真 app 有真 favicon 优先用，回退首字母块。
  点书签=新建/复用 web 标签加载该 URL。**不要**做成网页态书签栏，也**不要**加网页头。

### 4.6 收藏（星标 + 管理页 + Netscape 导入导出）（`bookmarks.ts` + `BookmarksPage.tsx`）
- **加/删**：地址栏的 `Star`（`⌘D` 也行）。默认落「书签栏」文件夹 → 出现在左侧栏收藏区里。
...
它的收藏/阅读模式（如果有）要按本文档 §4.4（无网页头）+ §4.5（侧栏折叠收藏区）+ §7 的砍除清单重新对齐，别照搬旧的。
...
- [ ] 左侧栏收藏区在「置顶」上方、默认收起、点标题行展开；展开后书签栏文件夹平铺 + 其余文件夹分组 + 管理入口。
- [ ] 网页态**无网页头**（网页内容直接铺满，URL / 安全指示 / 收藏都在左侧栏）。
- [ ] 点收藏里的书签=新标签打开；无 favicon 用首字母彩块（`FavChip`）。
