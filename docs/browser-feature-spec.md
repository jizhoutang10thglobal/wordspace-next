...
>
> 最后更新：2026-07-10（收藏区回侧栏折叠 + 删网页头定稿后）。
...
// state: { engine: EngineKey, homepage: string }；searchUrl(q) = 模板 %s ← encodeURIComponent(q)
```
...
**交互契约**
- **收起态**（默认）：仅一行。`▸ ChevronRight(13) + ⭐ Star(13, accent 色) + 「收藏」 + 数量(灰)`；
  行尾有管理入口 `Bookmark`(13)——**默认透明，hover 标题行或展开态才显现**（收起时保持干净）。
- 点标题行任意处 toggle 展开/收起（管理入口点击 stopPropagation，不触发 toggle）。
- **展开态**：caret 变 `ChevronDown`；下方列出书签：
...
- **点书签 = 开一个新网页标签**（前台激活）加载该 url，并记入历史；若正在历史/收藏/设置页则回主视图。
  （注意：**不是**在当前标签导航、也不去重已开标签——这是已上线的拍板行为；如要改成「已开则聚焦」
  见 §13 待拍板项。）
- 管理入口 → 收藏管理页（§4.9）。
...
**ui-demo 参考实现**：`ArcSidebar` 内 `.arc-fav` 区 + `FavChip` 组件；`favOpen` 是组件 state
（**不持久化**，刷新回到收起——见 §13）。`openBookmark(url,title)` = `openWebTab` + `history.record` + 回 `/docs`。

**真 app 后端设计**
- 收藏数据主进程持有（§4.9 存储），启动全量同步 renderer + 变更推送（`bookmarks:changed`）。
- 有 favicon 缓存（§10.4）时书签图标优先真 favicon，取不到回退 FavChip（算法照搬，纯前端）。
- 点书签：renderer 走「新建 web 标签 + 加载」流程（同 §4.4 / §10.2）；历史由主进程 `did-navigate`
  自动记（mock 手动 record 的补丁真 app 不需要）。
- 折叠态**建议持久化**（跟侧栏宽度等 UI 状态放一起），默认收起。
...
- 快捷瓦片 = 首字彩色 chip + 名称。ui-demo 是写死的 7 个演示位（招聘页/公司官网/Designer News/
  Glass 搜索/FlowDesk/维基百科/Example）——**演示内容，真 app 不照搬**；v1 可静态放常用站或从收藏取
  （§13 待拍板）。
...
- 每条书签一行：`Globe2` + 标题（就地改，失焦提交，空值回退为 url）+ url（去 scheme，title 提示全 url）+
  **文件夹下拉**（选中即移动）+ 打开 `ExternalLink`（新标签打开并记历史）+ 删除 `Trash2`。
...
已知小毛边（§13）：导入的非书签栏文件夹**盲追加**（重名会出两个）；toast 的 N 是解析数不是净新增数。
...
- **默认搜索引擎**：下拉（Glass 搜索 / Bing / Google / DuckDuckGo），说明「在地址栏打一句话（不是网址）时用它搜索」。
  选择即生效于 §5 的 normalize。
- **主页**：文本输入（失焦提交，空值回退默认），说明「新标签页 / 启动时打开」。
  **⚠ ui-demo 里此设置只存了没接线**（新标签页硬编码起始页）——见 §13 待拍板。

**真 app 后端设计**：并入 app 现有 settings 机制（键位建议 `browser.searchEngine` / `browser.homepage`）。
搜索引擎列表可扩展；**真 app 默认引擎必须换成真实引擎**（Glass 是 demo 虚构的，§13）。
...
- **图标索引（lucide，尺寸=px）**：
  ...
  收藏区 `ChevronRight·ChevronDown·Star·Bookmark 13`，FavChip 16×16；
...
**待拍板（不阻塞移植，实现时按默认走、标注给 Colin）**

1. **默认搜索引擎**：Glass 是 demo 虚构的，真 app 必须选真实默认（Bing？Google？）。→ Colin 拍。
2. **主页设置未接线**：ui-demo 存了 `homepage` 没消费。真 app 要么接上（⌘T/启动打开主页），要么把这行设置删掉。
3. **点收藏的打开语义**：现为「永远开新标签」（已上线拍板）。可选优化「已开该 url 则聚焦」。默认保持现状。
4. **收藏区折叠态持久化**：demo 刷新回收起。真 app 建议随 UI 状态持久化，默认收起。
5. 新标签页快捷瓦片内容：demo 写死 7 个演示位；真 app v1 建议取书签栏前 N 个或静态配置。
6. 导入的小毛边：重名文件夹盲追加、toast 计数=解析数。真 app 可顺手做成「按名合并 + 净新增数」，不算契约变更。
...
- [ ] 收藏区：默认收起一行（星标 accent、计数、hover 才显管理入口）；点行展开：书签栏平铺 + 文件夹分组 + FavChip；点书签=新标签打开。
...
- [ ] 设置：搜索引擎接进 normalize；主页项按 §13-2 的拍板处理。
...
| 2026-07-10 | Glass=虚构搜索引擎的口径保留（避免「克隆了 Bing」误导）；真 app 默认引擎待拍 |
