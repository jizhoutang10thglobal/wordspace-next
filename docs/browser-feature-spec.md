...
│ [🌐 地址栏………………… ☆]        │ ← omnibox（§4.2）
│ 收藏 3               ⧉ ▸    │ ← 收藏区，默认收起（§4.3）
│ 置顶 1                  ▾   │ ← 可折叠，默认展开（§4.4，三栏折叠统一）
│   ⌕ 员工手册            📌   │ ← TabStrip pinned
│ 标签页 2              +  ▾   │ ← 可折叠，默认展开（§4.4）
...
**三栏折叠统一（Wendi 2026-07-15）**：收藏 / 置顶 / 标签页三个区都可点栏标折叠（caret 右置、旋转、折叠态各自持久化 localStorage `ws-fav-open` / `ws-pinned-open` / `ws-tabs-open`，栏标带计数）。**收藏默认收起**（既有拍板），**置顶/标签页默认展开**（主导航，别一装就藏）——两个默认值不同是拍板。

**空态 CTA 位置（Wendi 2026-07-15）**：没打开任何文件夹时的「打开一个本地文件夹…」+「打开文件夹」按钮（`#sb-empty`）在侧栏**最底**（收藏/置顶/标签页之下），不再夹在收藏与置顶之间。真 app 有按钮；ui-demo 空态是 Library 底部纯文字（`arc-lib-empty`，无按钮）——记有意分歧。
...
- 一条输入框，值 = 激活标签的 `url`（切标签时同步重置）。placeholder「搜索,或输入网址」。聚焦时全选。
  **切标签强制结束编辑态（P2-3）**：打字打到一半用键盘切标签（`Ctrl+Tab`/`⌘1-9`，不触发 blur）时，
  强制结束打字态、**未提交输入丢弃**、地址栏复位为新标签 url——否则残留半截字会在新标签上误导航。
  只在「激活标签 key 真变了」时做；同标签的后台状态更新（title 推送）**不动**正在打的字（保留原「打字中
  不抢输入」守卫）。真 app 在 `syncChrome` 的激活同步点按 key 变化判定。
...
- **收起态**（默认）：仅一行，**穿侧栏栏标的衣服**——与「置顶/标签页/文档」同一套 editorial
  ...
  管理入口**常显**（2026-07-17 Wendi「收藏图标 hover 才显、标签页『+』常显,两者要统一」+ Colin 拍板
  「都一直出现」——推翻 07-13 的 hover 才显现；三栏行尾口径统一：**动作入口常显、caret 常显**）；
  hover 整行浅灰底、标题升 text-2。**行首不再有 caret 和 Star**（☆ 概念只留在地址栏；行首图标+彩色星标
  让同级 section 穿两套衣服，是「视觉乱」的根源——2026-07-13 按 Wendi 反馈定稿）。
- **对齐网格**：栏标文字左缘 8px（同置顶/标签页/文档），收藏项/文件夹名/空态文案左缘 10px
  （同标签行内容）。侧栏滚动列只允许这两条左缘。**计数与栏标同一光学中线、栏标→计数间距
  全侧栏统一 6px**（收藏/置顶/标签页三行一个口径；实现要点：padding 打在行容器上、不打在栏标
  子元素上——不对称纵向 padding 挂子元素会让计数上浮，2026-07-17 Colin 真机抓到，app e2e
  `sidebar-typography.spec.js` 几何门锁死）。
...
- **两区栏标可折叠**（三栏折叠统一，Wendi 2026-07-15，见 §3.1）：点栏标翻转、caret 右置旋转、栏标带计数、折叠态持久化（`ws-pinned-open` / `ws-tabs-open`，**默认展开**）。折叠只藏列表、栏标恒显示。收起态下新开标签只 +计数、不强制展开（同收藏区行为）。真 app `renderZones` 每次重建时从 localStorage 现读折叠态（内存变量随重渲丢失）。
- 每行：类型图标（网页=`Globe2`(13)，文档/文件=`FileText`(13)）+ 标题（省略号）+
  未保存圆点（临时文档）+ 置顶钮（`Pin`/已置顶 `PinOff`，12）+ 关闭钮 `X`(12)。
  **置顶标签有关闭钮（真 app，Colin 2026-07-14 拍板方案 B）**：置顶行的 `X` 点击 = **取消置顶并移除**
  （走 `removeTabRel`）；`⌘W` 仍**不销毁**置顶标签（键盘误关保护保留）。理由：这是 v0.4.5 起有意加、已上线
  半个多月的既有习惯，鼠标 `X` 是显式动作、误触率低。（ui-demo 侧仍「置顶无关闭钮」= Chrome 惯例，见 §13。）
  激活行高亮。
  **⚠ `⌘W` 命中置顶标签 ≠ 死无操作（Wendi 2026-07-17）**：关光普通标签后激活项会回落到置顶标签，
  此时 `⌘W` 若直接 no-op = 用户被困死（`⌘W` 零反馈、又没有关 app 的路）。**修正行为**：`⌘W` 命中置顶
  标签 → **取消激活、回起始页**（置顶标签留着、不销毁，下次点还在）；此时真·空态，再按一次 `⌘W` → 关窗口
  （§9 空态分流）。即"不销毁置顶"的保护保留，但 `⌘W` 始终有反馈、始终留一条关 app 的路（回起始页→关窗口）。
...
| `⌘W` | 关闭当前标签 | 置顶标签**不销毁**，但命中它时→回起始页（Wendi 2026-07-17，非死 no-op；§4.4/§9）；未保存文档先弹确认 |
...
| `⌘,` | 打开设置 | |
| `⌘\` | 收起/展开侧栏 | **全焦点域**（主层 / 文档编辑 iframe 内 / 网页 view）——经「视图」菜单加速器统一处理，是唯一通道；不另挂主层 keydown 或 web-tabs 转发（否则双触发=切两次） |
| `⌘R` | 刷新当前网页标签 | **仅网页标签**（重载当前 URL，等价导航条刷新钮，走其 disabled 守卫）；**文档标签有意 no-op**（防未保存编辑丢失，§2 决策）；起始页 url=null → no-op |
...
- 真 Electron 里 `⌘T/⌘W/⌘1-9` 建议同时注册**应用菜单加速键**（网页 demo 里这些会被浏览器吞，
  Electron 菜单级注册才稳）。
- **可发现性两档**（Wendi 2026-07-16 反馈「快捷键不可见=没人用」，ui-demo 定稿后两侧同步）：
  ① **简洁 tooltip 一直有**：有快捷键的控件挂 native `title`，格式 `动作 ⌘X`（mac 字形；非 mac 展开成
  `Ctrl+X`——静态 title 启动时归一，动态 title 各创建处走 `kbd()`/`__wsKbd`）。已挂：收起/展开侧栏 `⌘\`、
  查找文件 `⌘P`、收藏 `⌘D`、刷新 `⌘R`、关标签 `⌘W`、新建标签 `⌘T`、另存为 `⌘⇧S`。`⌘F`/纯 `⌘S` 纯键盘
  触发、无独立控件，不挂（Colin 拍板不做）。
  ② **教学气泡每操作只教一次**：用户**鼠标**点了有快捷键的操作 → hint toast「下次可以用 ⌘X」（灯泡 +
  accent 色条，4.2s），`localStorage ws-coached-ops` 记住、一辈子只弹一次；键盘/程序化触发不弹（⌘R 菜单
  路径走 `navReload.click()`，`isTrusted=false` 过滤）。已接：收起/展开侧栏、刷新、新建标签、关标签。
  门 `e2e/tabs.spec.js`「快捷键 tooltip」+「快捷键教学气泡」。（`⌘/` 总表面板仍在欠账。）
...
- [ ] 收藏区：默认收起一行（栏标样式同置顶/标签页、计数、**管理入口常显**（07-17 统一拍板）、行尾 caret 常显展开旋转、无 Star）；对齐网格=栏标 8px/内容 10px；点行展开：书签栏平铺 + 文件夹分组 + FavChip；点书签=**已开则聚焦，否则新标签**；折叠态记住上次。
...
- [ ] 标签：置顶有关闭钮（真 app，点击=取消置顶并移除；§13/§4.4）、⌘W 守置顶、未保存先确认、关闭激活→**相邻**、拖拽跨组=置顶语义、⌘⇧T（15 条栈）、Ctrl+Tab 循环、⌘1-9、⌘T 二合一 modal、会话恢复（web 标签懒加载）。
