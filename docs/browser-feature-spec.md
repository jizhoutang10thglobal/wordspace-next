...
- 真 Electron 里 `⌘T/⌘W/⌘1-9` 建议同时注册**应用菜单加速键**（网页 demo 里这些会被浏览器吞，
  Electron 菜单级注册才稳）。
- **可发现性：有快捷键的控件挂 native `title` 提示**（格式 `动作 (Cmd+X)`，悬停可见）——快捷键不可见=没人用
  （Wendi 2026-07-16 反馈）。已挂：收起侧栏 `⌘\`、查找文件 `⌘P`、收藏 `⌘D`、刷新 `⌘R`、关标签 `⌘W`、
  新建标签 `⌘T`、另存为 `⌘⇧S`。`⌘F`（文档内查找）/纯 `⌘S`（保存）纯键盘触发、无独立控件，不挂。
  门 `e2e/tabs.spec.js`「快捷键 tooltip」。（更主动的教学气泡 / `⌘/` 总表面板见欠账，本次未做。）
