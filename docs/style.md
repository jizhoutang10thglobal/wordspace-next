...
| token | 值(亮) | 暗色(初稿) | 用途 |
|---|---|---|---|
| `--c-bg` | `#FFFFFF` | `#2B2724` | 文档纸面 |
| `--c-bg-chrome` | `#FAFAF9` | `#262220` | 标题栏/标签条(stone-50) |
| `--c-bg-sunken` | `#F5F5F4` | `#1F1B18` | 侧栏等下沉面板(stone-100) |
| `--c-bg-rail` | `#EDECEB` | `#17130F` | 最左图标轨 |
| `--c-surface` | `#FFFFFF` | `#2B2724` | 卡片/菜单/弹窗 |
| `--c-border` / `-strong` | `#E7E5E4` / `#D6D3D1` | `#3A3531` / `#4A4440` | 细边 / 强边 |
| `--c-divider` | `#EFEDEB` | `#332E2B` | 分隔线 |
| `--c-text` / `-2` / `-3` | `#292524` / `#57534E` / `#A8A29E` | `#EDE9E6` / `#B3ACA6` / `#78716C` | 正文 / 次要 / 占位 |
| `--c-ink` / `-hover` | `#1C1917` / `#292524` | `#ECE7E3` / `#F7F3F0` | **墨色控件底**(primary 钮/switch on/tooltip);暗态反转为亮墨 |
| `--c-accent` | `#1D6FBF` | `#5C9FDF` | 墨青蓝:链接/选中 tint/焦点圈。**不做大面积底色**;暗态提亮一档保对比度 |
| `--c-hover` / `--c-active` | `rgba(28,25,23,.05)` / `.09` | `rgba(237,233,230,.06)` / `.10` | hover / 按下底色(墨的透明度,不是灰) |
| `--c-success` / `warn` / `danger` | `#15803D` / `#B45309` / `#B91C1C` | `#46B366` / `#D98A3A` / `#E5645F` | 语义色(饱和度压低的「印章色」) |

原则:**primary 按钮是墨不是蓝**。蓝只出现在"指出去"的东西上(链接/选中/焦点)。
投影一律用暖墨基底 `rgba(28,25,23,·)`,禁止冷黑 `rgba(0,0,0,·)`(暗态例外:投影底改用重黑透明做层分离,仍**不发光**)。

**暗色变体(「浅色/深色/跟随系统」三态外观模式,机器可读值在 `tokens.css` 的 `:root[data-theme="dark"]`)**:
⚠ 暗色列是**工程初稿,待 Wendi 真机拍板**(R12,palette 观感终审权在 Wendi)。语义映射:纸=暖黑 stone 深灰
(非冷黑,守暖墨基底);墨色控件反转为亮墨;accent 提亮一档;投影用更重的黑透明,绝不借暗态加 glow。
文本×背景对比度底线(正文≥4.5:1、大字≥3:1)由 `test/appearance-contrast-pairs.js` 配对清单 + 门把关。
type/radius/motion 是主题无关量,暗色块不重复(不是遗漏)。
