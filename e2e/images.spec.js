...
test('斜杠插入：图真渲染 + 降采样≤1600 + 磁盘字节过校验器判 conform', async () => {
  ...
  // 磁盘字节 conform（比活 DOM serialize 强一档）：poll 到落盘为止（替死等 2200ms,flake-proof,U3-B1）→ readFile → JSDOM → classify.conform
  await expect.poll(() => fs.readFile(docPath, 'utf8'), { timeout: 6000 }).toMatch(/<img[^>]+src="data:image\//);
...
test('加说明：figure/figcaption canonical + 磁盘 conform；说明里退格不删整块', async () => {
  ...
  await expect.poll(async () => (await fs.readFile(docPath, 'utf8')).replace(/\n/g, ''), { timeout: 6000 }).toMatch(/<figure><img[^>]+><figcaption>海报配图<\/figcaption><\/figure>/); // U3-B1：poll 落盘,替死等 2200ms
...
test('重开文档：内联 data: 图冷启动仍真渲染（持久化）', async () => {
  ...
  await insertViaSlash('p1');
  await pollImg('插入后图未渲染');
  await expect.poll(() => fs.readFile(docPath, 'utf8'), { timeout: 6000 }).toMatch(/src="data:image\//); // U3-B1：poll 落盘,替死等 2200ms
...
test('选中框 accent 蓝（暗色可见）：选中图的 box-shadow 是蓝环、不是暗色下隐身的黑环', async () => {
  ...
  await pollImg('插入后图未渲染');
  await frame.locator('img').click();
  await expect(frame.locator('img[data-ws2-selected]')).toBeVisible(); // U3-B1：等选中标记,替死等 150ms
...
test('变异自检（门有牙）：把 img.src 打坏后，同一 checkImg 谓词必翻红', async () => {
  ...
  await expect.poll(async () => (await checkImg()).pass, { timeout: 4000 }).toBe(false); // U3-B1：poll 到坏 src 判假,替死等 400ms
