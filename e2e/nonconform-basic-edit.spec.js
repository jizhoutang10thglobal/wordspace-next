...
test('悬停不出块 chrome（Wendi 2026-07-14 撤悬停框）：mousemove 扫过内容 → 无 .nce-hover/.nce-lock', async () => {
  await launch();
  await openDoc('wild.html', WILD);
  // 扫过图片和文字（原悬停删除/🔒 的触发路径）——现在不该出现任何悬停浮层
  for (const sel of ['#pic', '#p1']) {
    const box = await frame.locator(sel).boundingBox();
    await page.mouse.move(box.x + box.width / 2, box.y + box.height / 2);
    await page.waitForTimeout(200);
  }
  expect(await page.locator('.nce-hover').count()).toBe(0);
  expect(await page.locator('.nce-lock').count()).toBe(0);
});
