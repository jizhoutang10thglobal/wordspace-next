// U3 收尾截图探针（不入库）：行作用域菜单 vs 块作用域菜单 + 行级删除结果。
...
const CLIP = { x: 180, y: 20, width: 900, height: 420 };
...
const shot = async (name) => { await page.waitForTimeout(250); await page.screenshot({ path: path.join(SHOTS, name), clip: CLIP }); };
...
test('U3 截图：行作用域菜单', async () => {
  ...
  await shot('u3-row-menu.png');
});

test('U3 截图：块作用域菜单（Esc 灰选）', async () => {
  await launch();
  await openDoc(FIX);
  ...
  await frame.locator('.ws-grip').click();
  await expect(frame.locator('.ws-blockmenu')).toBeVisible();
  await shot('u3-block-menu.png');
});

test('U3 截图：行级删除结果', async () => {
  await launch();
  await openDoc(FIX);
  await frame.locator('#r2').hover();
  await page.waitForTimeout(150);
  ...
  await shot('u3-row-deleted.png');
  console.log('U3 SHOTS DONE');
});
