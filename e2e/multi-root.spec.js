...
test('MR-load 两段式添加：读盘期间显示加载行、读完填树（WS2_SLOW_TREE_MS，变异敏感）', async () => {
  // 大文件夹全量扫描 4-5s 卡在 IPC 回复上时用户干等无反馈（Wendi 2026-07-14）。两段式：先回根 + 加载行，再填树。
  await launch({ WS2_FOLDER_IN: wsA, WS2_SLOW_TREE_MS: '1200' });
  await page.click('#home-open-folder');
  // 第一段：根标题立刻出现 + 「正在读取…」加载行（变异门：去掉加载行渲染 / 两段式回退到同步 await 这里都翻红）
  await expect(rootHeads()).toHaveCount(1);
  await expect(page.locator('.sb-loading')).toBeVisible();
  await expect(page.locator('.sb-loading')).toHaveText('正在读取文件夹…');
  await expect(page.locator('.sb-file[data-rel="a.html"]')).toHaveCount(0); // 读盘中，文件还没上树
  // 第二段：读完 → 加载行消失 + 文件出现
  await expect(page.locator('.sb-loading')).toHaveCount(0, { timeout: 6000 });
...
test('MR-3 嵌套检测：same/child 不重复加（toast 解释），parent 弹「并入并添加」+ 标签 rebase 不关', async () => {
  ...
  await page.click('#sb-add-root');
  await expect(page.locator('.sb-toast', { hasText: '已经打开' })).toBeVisible();
  await expect(rootHeads()).toHaveCount(2);
  // child：选 A/素材 → toast、节数不变
  await setFolderSeam(path.join(wsA, '素材'));
  await page.click('#sb-add-root');
  await expect(page.locator('.sb-toast', { hasText: '里了' })).toBeVisible();
  await expect(rootHeads()).toHaveCount(2);
...
test('MR-5b 失联根自动复活探测：改回原路径后免手点「重新定位」自愈（P3-08，变异敏感）', async () => {
  const [, rb] = await openTwoRoots();
  await expect(fileRow(rb, 'b.html')).toBeVisible();
  // 运行中把 B 挪走 → 触发一次读树（ws-read-tree 读到 null + 路径不可达 → 主进程标失联并广播）
  const moved = path.join(tmp, '资料库-临时挪走');
  await fs.rename(wsB, moved);
  await page.evaluate((id) => window.ws2.wsReadTree(id), rb);
  await expect(page.locator(`.sb-root-head.sb-root-missing[data-root="${rb}"]`)).toBeVisible({ timeout: 8000 });
  await expect(fileRow(rb, 'b.html')).toHaveCount(0); // 失联期间树行不在
  // 把目录改回一模一样的原路径 → 不做任何手动操作，仅等复活探测（5s 一次）自愈
  await fs.rename(moved, wsB);
  await expect(page.locator(`.sb-root-head.sb-root-missing[data-root="${rb}"]`)).toHaveCount(0, { timeout: 9000 });
  await expect(fileRow(rb, 'b.html')).toBeVisible({ timeout: 3000 }); // 树真的回来了
});

test('P3-06 同名跨根消歧：双根同名都打开→标签带各自根名后缀 + title；关一个→后缀消失（变异敏感）', async () => {
  const [ra, rb] = await openTwoRoots();
  await page.click(`.sb-dir[data-root="${ra}"][data-rel="素材"]`); // 展开 甲方项目/素材
  await fileRow(ra, '素材/同名.html').click();
  await fileRow(rb, '同名.html').click();
  // 冲突：两条标签都带「— 根名」淡色后缀
  await expect(tabRow(ra, '素材/同名.html').locator('.sb-tab-rootsuffix')).toHaveText('— 甲方项目');
  await expect(tabRow(rb, '同名.html').locator('.sb-tab-rootsuffix')).toHaveText('— 资料库');
  // title tooltip = 根名 / rel（普适，冲突与否都有）
  await expect(tabRow(ra, '素材/同名.html').locator('.sb-name')).toHaveAttribute('title', '甲方项目 / 素材/同名.html');
  await expect(tabRow(rb, '同名.html').locator('.sb-name')).toHaveAttribute('title', '资料库 / 同名.html');
  // 关掉 B 的同名 → 冲突消失，A 的后缀也没了（无冲突不加长）
  await tabRow(rb, '同名.html').hover();
  await tabRow(rb, '同名.html').locator('.sb-tab-close').click();
  await expect(tabRow(rb, '同名.html')).toHaveCount(0);
  await expect(tabRow(ra, '素材/同名.html').locator('.sb-tab-rootsuffix')).toHaveCount(0);
});
...
test('MR-13 跨盘(EXDEV seam)：toast 提示、文件纹丝不动、标签不变（变异敏感门）', async () => {
  ...
  await dndTo(ra, 'a.html', `.sb-root-head[data-root="${rb}"]`);
  await expect(page.locator('.sb-toast', { hasText: '不同的磁盘' })).toBeVisible();
  // 文件没动、标签还在 A、B 没冒出来
