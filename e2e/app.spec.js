...
// baseline 页边距（Wendi/§0 决策2）：裸文档四周留白 + 入盘 data-ws-schema-css；不加 max-width 居中窄栏。
test('baseline: 文档四周留白（页边距）+ 入盘 + 不加窄栏', async () => {
  await launch();
  ...
  expect(maxW, 'baseline 不该加 max-width 居中窄栏（§0：不套装饰）').toBe('none');
  const html = await serialize();
