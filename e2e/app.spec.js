...
const setSel = (a, b, c, d) => frame.locator('body').evaluate((body, [a, b, c, d]) => {
...
test('回归：跨块选区 Backspace 能删并合并（Wendi Bug4/5）', async () => {
  ...
  await frame.locator('#p1').click(); await page.waitForTimeout(120); // 聚焦 iframe
  await setSel('p1', 3, 'p2', 3);   // 选 "111"+换行+"BBB"（跨 p1/p2）
  await page.keyboard.press('Backspace'); await page.waitForTimeout(200);
  expect(await layoutOf()).toEqual(['P:AAA222', 'P:CCC']); // 起末块裁剪 + 合并；中间无；p3 留
});

test('回归：无编辑态拖选（单块）Backspace 能删（Wendi Bug4/5）', async () => {
  await launch();
  await openDoc(DEL_DOC);
  await frame.locator('#p1').click(); await page.waitForTimeout(120);
  await setSel('p1', 3, 'p1', 6);   // 选 p1 的 "111"
  await page.keyboard.press('Backspace'); await page.waitForTimeout(200);
  expect((await layoutOf())[0]).toBe('P:AAA');
});
...
test('回归：跨块 Cmd+X 剪切——删除 + 复制进剪贴板（Wendi Bug6）', async () => {
  ...
  await frame.locator('#p1').click(); await page.waitForTimeout(120);
  await setSel('p1', 3, 'p2', 3);
  await page.keyboard.press('Meta+x'); await page.waitForTimeout(250);
  expect(await layoutOf(), '跨块剪切没删掉选区').toEqual(['P:AAA222', 'P:CCC']);
