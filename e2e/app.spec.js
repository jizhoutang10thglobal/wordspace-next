...
test('常驻工具栏：开文档即显示；双击进编辑、选区点加粗跨帧生效', async () => {
  await launch(FIXTURE);
  await openDoc();
  await expect(page.locator('#toolbar')).toBeVisible();
  await frame.locator('#p1').dblclick();    // 进文字编辑（contenteditable）
  await frame.locator('#p1').selectText();  // 选中整段文字（range）
...
test('工具栏标题下拉：选中段落转标题 2（元素级，无需进编辑）', async () => {
  await launch(FIXTURE);
  await openDoc();
  await frame.locator('#p2').click(); // 画布单击 = 选中元素
...
test('工具栏复制块：选中段落复制一份（元素级）', async () => {
  ...
  await frame.locator('#p1').click(); // 选中
