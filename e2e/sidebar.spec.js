...
test('侧栏启动即显示（browser UX 重做：omnibox 常驻）；开屏 NewTab、打开工作区后树出现', async () => {
  // browser-tabs UX 重做后：侧栏常驻（地址栏要一直在）、开屏 = NewTab 空页面（打开文件夹入口在其上）。
  await expect(page.locator('#sidebar.sb-on')).toBeVisible();
  await expect(page.locator('#web-newtab')).toBeVisible();
  await expect(page.locator('#nt-open-folder')).toBeVisible();
  // 打开后：文件树出现
  await openWorkspace();
  await expect(page.locator('.sb-file[data-rel="a.html"]')).toBeVisible();
});
