...
test('文件夹卡尺点击：收起/展开子节点', async () => {
  await openWorkspace();
  await expect(page.locator('.sb-file[data-rel="数据/b.html"]')).toBeVisible();
  await page.locator('.sb-dir[data-rel="数据"]').click(); // 收起
  await expect(page.locator('.sb-file[data-rel="数据/b.html"]')).toHaveCount(0);
  await page.locator('.sb-dir[data-rel="数据"]').click(); // 再展开
  await expect(page.locator('.sb-file[data-rel="数据/b.html"]')).toBeVisible();
});

test('空文件夹展开显示「空文件夹」', async () => {
  await openWorkspace();
  // 文件夹默认全展开，「空目录」其下直接有占位文案
...
test('点非 .html（README，无扩展名）→ 经 ws-open-external 交系统程序、不进编辑器', async () => {
  await openWorkspace();
  // window.ws2 是 contextBridge 冻结对象、改不了，所以在主进程桩掉 shell.openPath（同时避免真启动宿主程序）。
  // 这反而更强：验到的是主进程 ws-open-external 把 README 经 assertInsideWorkspace 解析成的真实绝对路径。
  ...
  await expect(page.locator('#home')).toBeVisible(); // 没开文档，还在空态
