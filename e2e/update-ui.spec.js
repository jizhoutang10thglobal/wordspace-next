...
test('「后台下载」关掉面板后：进度推送不再弹回，状态跃迁（就绪）才重新弹', async () => {
  ...
  await expect(card).toHaveCount(0);
  // 后续进度推送（手动流里每 ~200ms 一条）不许把面板弹回来——Colin 2026-07-17 实踩：一关立刻被打脸重开
  ...
  await expect(card).toHaveCount(0);
  await expect(page.locator('#sb-update-txt')).toContainText('42%'); // 后台下载：pill 仍在跟进度
  // 状态跃迁 downloading → ready：这才是「全程跟进」该弹的里程碑
...
test('安装授权说明按归属 uid 判定（非 W_OK）：root-owned 才弹、healthy 静默', async () => {
  // 直调 maybeExplainInstallAuth（真 install handler dev 态提前 return，绕不到它）。
  // 判据必须是「bundle 属主是不是你」——不能用 fs.access(W_OK)：/Applications 里已签名 app 的
  // W_OK 恒不可写（SIP/App Management），会把归属已修好的机器误判成 root-owned、每次更新误弹（Colin 07-17）。
  const run = (owned, resp) => app.evaluate(async ({ }, { owned, resp }) => {
    if (owned == null) delete process.env.WS2_BUNDLE_OWNED; else process.env.WS2_BUNDLE_OWNED = owned;
    if (resp == null) delete process.env.WS2_AUTH_DIALOG_RESPONSE; else process.env.WS2_AUTH_DIALOG_RESPONSE = String(resp);
    const before = globalThis.__ws2UpdateSim.calls.authShown;
    const proceed = await globalThis.__ws2UpdateSim.authExplain();
    return { proceed, shown: globalThis.__ws2UpdateSim.calls.authShown - before };
  }, { owned, resp });

  // healthy（属主=你）：不弹、放行安装
  expect(await run('self', null)).toEqual({ proceed: true, shown: 0 });
  // root-owned + 选「继续安装（需授权）」(0)：弹说明、放行安装
  expect(await run('root', 0)).toEqual({ proceed: true, shown: 1 });
  // root-owned + 选「去官网重装」(1)：弹说明、本次不装
  expect(await run('root', 1)).toEqual({ proceed: false, shown: 1 });
  // root-owned + 取消(2)：弹说明、本次不装
  expect(await run('root', 2)).toEqual({ proceed: false, shown: 1 });

  await app.evaluate(() => { delete process.env.WS2_BUNDLE_OWNED; delete process.env.WS2_AUTH_DIALOG_RESPONSE; });
});
