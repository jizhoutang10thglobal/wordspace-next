...
// Wendi 2026-07-17：关光普通标签后激活项回落到置顶标签，⌘W 什么都不发生=被困死、又没有关 app 的路。
// 修后 ⌘W 命中置顶标签 → 回起始页（置顶标签留着、不销毁）；此时真·空态，再按一次才关窗口。
test('Cmd+W 分层：命中置顶标签先回起始页（置顶保留），再按一次才关窗口', async () => {
  await openWorkspace();
  // 开 a.html（普通标签、激活）→ 钉住（移进置顶区，仍激活、仍开着）→ 现在没有普通标签了（复现 Wendi 状态）
  await page.click('.sb-file[data-rel="a.html"]');
  await expect(page.locator('#sb-tabs .sb-tab[data-rel="a.html"]')).toHaveClass(/is-active/);
  await page.locator('#sb-tabs .sb-tab[data-rel="a.html"]').hover();
  await page.locator('#sb-tabs .sb-tab[data-rel="a.html"] .sb-tab-pin').click();
  await expect(page.locator('#sb-pinned .sb-tab[data-rel="a.html"]')).toBeVisible(); // 进了置顶区
  await expect(page.locator('#sb-tabs .sb-tab')).toHaveCount(0);                     // 没有普通标签
  await expect(page.locator('#sb-pinned .sb-tab[data-rel="a.html"]')).toHaveClass(/is-active/); // 置顶项仍激活
  await expect(page.locator('#doc-frame')).toBeVisible();

  // 第一次 ⌘W：命中置顶 → 回起始页，窗口绝不能动、置顶不能丢
  await menu('close-tab');
  await expect(page.locator('#home')).toBeVisible();                                   // 露出起始页
  await expect(page.locator('#doc-frame')).toBeHidden();                               // 文档收起
  await expect(page.locator('#sb-pinned .sb-tab[data-rel="a.html"]')).toBeVisible();   // ★置顶标签还在（没被 ⌘W 销毁）
  await expect(page.locator('#sb-pinned .sb-tab[data-rel="a.html"]')).not.toHaveClass(/is-active/); // 已取消激活
  expect(await winVisible()).toBe(true);                                              // 窗口没关

  // 第二次 ⌘W：真·空态 → 关窗口（mac 隐藏驻留 / 其他平台退出，与既有空态门同分流）
  if (IS_MAC) {
    await menu('close-tab');
    await expect.poll(winVisible).toBe(false);   // 窗口藏了
    await app.evaluate(({ app: a }) => { a.emit('activate'); }); // 点 Dock 唤回
    await expect.poll(winVisible).toBe(true);
    await expect(page.locator('#sb-pinned .sb-tab[data-rel="a.html"]')).toBeVisible(); // 置顶跨驻留仍在
  } else {
    const closed = app.waitForEvent('close');
    await menu('close-tab');
    await closed; // Linux/Windows：真·空态关窗即退
    neuterApp();
  }
});
