...
test('popover：开(#nav-downloads → .dlp + 摘 view) → 逐状态操作按 §4.11 → Esc/veil 关 → 清空只清终态', async () => {
  await launch();
  await openReadyWebTab();
  await triggerDownload(base + '/dl');
  await expect.poll(async () => (await dlList()).some((e) => e.state === 'completed'), { timeout: 8000 }).toBe(true);
  // 开 popover：.dlp-overlay 进 OVERLAY_SEL → 主进程 view 被摘（DOM 卡片才盖得住原生 view）。
  await page.locator('#nav-downloads').click();
  await expect(page.locator('.dlp')).toBeVisible();
  await expect.poll(() => attachedCount(), { timeout: 4000 }).toBe(0); // view 真摘（强断言：非查 .dlp 存在，查原生 view 已让位）
  // 逐状态操作（completed）：在访达中显示 + 移除；**无**取消（spec §4.11 表）。查 title 精确匹配 i18n 文案。
  ...
  // 重开 → 点 veil 关（网页区被原生 view 吞 click，veil 是唯一无损「点外面关」）。
  await page.locator('#nav-downloads').click();
  await expect(page.locator('.dlp')).toBeVisible();
  await page.locator('.dlp-veil').click({ position: { x: 5, y: 700 } }); // 点卡片外的 veil 空白处
  await expect(page.locator('.dlp')).toHaveCount(0);
...
test('收起态 toast：侧栏收起下触发下载 → 开始 toast 在视口内可见（boundingBox 兜底反馈）', async () => {
  await launch();
  await openReadyWebTab();
  await page.click('#sb-toggle'); // 收起侧栏（沉浸态）
  await expect(page.locator('#sidebar')).toHaveClass(/is-collapsed/);
  await triggerDownload(base + '/dl');
  // 开始下载的 neutral toast（收起态唯一可见反馈，spec §4.11「通知」）：dlStarted='正在下载 {name}'。
