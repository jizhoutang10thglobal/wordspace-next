...
test('U26d(精确)：跨两个 toggle（A 体内→B 体内）Delete → A 存活体内清空、B 解散消失、中间块删', async () => {
  ...
  const after = await frame.locator('body').evaluate(() => {
    ...
    return {
      det: dets.length,
      aSummary: dets[0] ? dets[0].querySelector('summary').textContent : null,
      aBodyEmpty: dets[0] ? [...dets[0].children].filter((c) => c.tagName !== 'SUMMARY' && !c.hasAttribute('data-ws2-ui')).every((b) => (b.textContent || '').trim() === '') : null,
      mid: !!document.getElementById('mid'),
      pre: (document.getElementById('pre') || {}).textContent,
      post: (document.getElementById('post') || {}).textContent,
    };
  });
  // 精确契约(Colin 2026-07-24 二轮)：A 的 summary 在选区之前未被罩 → A 存活(体内删空补 p 铁则)；
  // B 的 summary 被罩住 → B 解散、被罩体内块删光 → B 消失。
  expect(after.det, 'A 存活(summary 未被罩)、B 解散 → 剩 1 个 toggle').toBe(1);
  expect(after.aSummary, 'A 的标题不动').toBe('甲');
  expect(after.aBodyEmpty, 'A 体内被罩块删光 → 铁则补空块').toBe(true);
  expect(after.mid, '夹在中间的段落(全罩)删').toBe(false);
  expect(after.pre).toBe('前段');
  expect(after.post).toBe('后段');
  expect(await conform23(await serialize23())).toBe(true);
});
...
// ── 精确选区契约（Colin 2026-07-24 二轮拍板：「鼠标从哪到哪,高亮/删除就从哪到哪,不加不减」）──
test('P1(精确高亮)：端点块部分选中不整行蓝，完全罩住的块和 summary 行才蓝', async () => {
  await openDoc('<p id="pre">前段文字甲乙丙丁</p><details open id="dt"><summary id="sm">标题S</summary><p id="bd">体内容BBBB</p><p id="b2">体二CCCC</p></details><p id="post">后段</p>');
  await frame.locator('#pre').click();
  // pre 中间(offset 4) → bd 中间(offset 3)：pre 部分、summary 全罩、bd 部分、b2 未涉
  await frame.locator('body').evaluate((body) => {
    const doc = body.ownerDocument;
    const a = body.querySelector('#pre').firstChild, b = body.querySelector('#bd').firstChild;
    const r = doc.createRange(); r.setStart(a, 4); r.setEnd(b, 3);
    const sel = doc.getSelection(); sel.removeAllRanges(); sel.addRange(r);
  });
  await page.waitForTimeout(300);
  const st = await frame.locator('body').evaluate(() => ({
    pre: document.getElementById('pre').hasAttribute('data-ws2-rangesel'),
    det: document.getElementById('dt').hasAttribute('data-ws2-rangesel'),
    sm: document.getElementById('sm').hasAttribute('data-ws2-rangesel'),
    bd: document.getElementById('bd').hasAttribute('data-ws2-rangesel'),
    b2: document.getElementById('b2').hasAttribute('data-ws2-rangesel'),
    post: document.getElementById('post').hasAttribute('data-ws2-rangesel'),
  }));
  expect(st.pre, '起点块部分选中 → 不整行蓝(原生文字高亮)').toBe(false);
  expect(st.det, '部分进入的 toggle 不整块上卷标蓝').toBe(false);
  expect(st.sm, 'summary 行整个被罩 → 整行蓝(预示解散)').toBe(true);
  expect(st.bd, '末端体内块部分选中 → 不整行蓝').toBe(false);
  expect(st.b2, '选区外的体内块不蓝').toBe(false);
  expect(st.post, '选区外的后段不蓝').toBe(false);
});

test('P2(精确删除)：外面段落中→体内块中 Delete → 精确删+解散+两头接一条线', async () => {
  await openDoc('<p id="pre">前段文字甲乙丙丁</p><details open id="dt"><summary id="sm">标题S</summary><p id="bd">体内容BBBB</p><p id="b2">体二CCCC</p></details><p id="post">后段</p>');
  await frame.locator('#pre').click();
  await frame.locator('body').evaluate((body) => {
    const doc = body.ownerDocument;
    const a = body.querySelector('#pre').firstChild, b = body.querySelector('#bd').firstChild;
    const r = doc.createRange(); r.setStart(a, 4); r.setEnd(b, 3); // 「前段文字|甲乙丙丁」→「体内容|BBBB」
    const sel = doc.getSelection(); sel.removeAllRanges(); sel.addRange(r);
  });
  await page.waitForTimeout(250);
  await page.keyboard.press('Delete');
  ...
  const st = await frame.locator('body').evaluate(() => ({
    ...
    pre: (document.getElementById('pre') || {}).textContent,
    b2: (document.getElementById('b2') || {}).textContent,
    b2Parent: document.getElementById('b2') ? document.getElementById('b2').parentElement.tagName : null,
    post: (document.getElementById('post') || {}).textContent,
  }));
  expect(st.det, 'summary 被罩 → toggle 解散(壳删)').toBe(0);
  expect(st.pre, '前段裁尾 + 体内块尾巴并入(两头接一条线)').toBe('前段文字BBBB');
  expect(st.b2, '幸存体内块内容零丢失').toBe('体二CCCC');
  expect(st.b2Parent, '幸存体内块提升为顶层块').toBe('BODY');
  expect(st.post).toBe('后段');
  expect(await conform23(await serialize23())).toBe(true);
});

test('P3(精确删除)：体内块中→外面段落中 Delete → toggle 存活、跨壁不并', async () => {
  await openDoc('<details open id="dt"><summary id="sm">标题S</summary><p id="bd">体内容BBBB</p></details><p id="post">后段文字甲乙丙丁</p>');
  await frame.locator('#bd').click();
  await frame.locator('body').evaluate((body) => {
    const doc = body.ownerDocument;
    const a = body.querySelector('#bd').firstChild, b = body.querySelector('#post').firstChild;
    const r = doc.createRange(); r.setStart(a, 3); r.setEnd(b, 4); // 「体内容|BBBB」→「后段文字|甲乙丙丁」
    const sel = doc.getSelection(); sel.removeAllRanges(); sel.addRange(r);
  });
  await page.waitForTimeout(250);
  await page.keyboard.press('Delete');
  ...
  const st = await frame.locator('body').evaluate(() => {
    ...
    return {
      det: !!det,
      sm: det ? det.querySelector('summary').textContent : null,
      bd: (document.getElementById('bd') || {}).textContent,
      bdInDet: document.getElementById('bd') ? !!document.getElementById('bd').closest('details') : null,
      post: (document.getElementById('post') || {}).textContent,
    };
  });
  expect(st.det, 'summary 未被罩 → toggle 存活').toBe(true);
  expect(st.sm, '标题不动').toBe('标题S');
  expect(st.bd, '体内起块裁尾').toBe('体内容');
  expect(st.bdInDet, '体内起块留在 toggle 里(跨壁不并、不吸不漏)').toBe(true);
  expect(st.post, '外面末块裁头、自留(不被吸进 toggle)').toBe('甲乙丙丁');
  expect(await conform23(await serialize23())).toBe(true);
});

test('P4(以上块为准)：列表项中→下面段落中 Delete → 段落剩余并进末项、样式随上块', async () => {
  await openDoc('<ul id="lst"><li>甲项内容</li><li>乙项内容</li></ul><p id="pp">段落文字甲乙丙丁</p><p id="post">后段</p>');
  await frame.locator('#lst li').first().click();
  await frame.locator('body').evaluate((body) => {
    const doc = body.ownerDocument;
    const li2 = body.querySelectorAll('#lst li')[1].firstChild, pp = body.querySelector('#pp').firstChild;
    const r = doc.createRange(); r.setStart(li2, 2); r.setEnd(pp, 4); // 「乙项|内容」→「段落文字|甲乙丙丁」
    const sel = doc.getSelection(); sel.removeAllRanges(); sel.addRange(r);
  });
  await page.waitForTimeout(250);
  await page.keyboard.press('Delete');
  await page.waitForTimeout(250);
  const st = await frame.locator('body').evaluate(() => ({
    liTexts: [...document.querySelectorAll('#lst li')].map((l) => l.textContent),
    pp: !!document.getElementById('pp'),
    post: (document.getElementById('post') || {}).textContent,
  }));
  expect(st.liTexts, '段落剩余「甲乙丙丁」并进末项(样式以上块为准=还是列表项)').toEqual(['甲项内容', '乙项甲乙丙丁']);
  expect(st.pp, '下段落被并掉').toBe(false);
  expect(st.post).toBe('后段');
  expect(await conform23(await serialize23())).toBe(true);
});
