...
test('STICKY-1 滚进深层子树 → 根标题+三级祖先吸顶（正确链 + 贴可视区顶 + 阴影真被画）', async () => {
  await scrollFileToTop(`${DEEP}/华东区/明细/6月.html`, 40); // 6月.html 到顶下 40px → 根+归档/华东区/明细 吸顶其上
  await page.waitForTimeout(120);
  const s = await stickyState();
  // 多根后根标题行（depth=-1）是链最外层：滚多深都看得见「在哪个文件夹里」
  expect(s.count).toBe(4);
  expect(s.rows[0]).toContain('workspace'); // 根标题行（根名=文件夹名）
  // 其下自上而下 = compact 归档链 → 华东区 → 明细（= 折线处 6月.html 的真实祖先）
  expect(s.rows[1]).toContain('归档');
  expect(s.rows[1]).toContain('复盘');
  expect(s.rows[2]).toBe('华东区');
  expect(s.rows[3]).toBe('明细');
...
test('STICKY-2 变异探针：滚回顶部 → 浮层清空隐藏（证明滚动驱动、非常驻元素）', async () => {
  ...
  await page.waitForTimeout(120);
  expect((await stickyState()).count).toBe(4); // 深处：有吸顶（根标题+三级）
  // 滚回顶
...
test('STICKY-3 旁支排除：滚进华南区（华东区的兄弟支）→ 吸顶归档+华南区，不吸华东区/明细', async () => {
  ...
  await page.waitForTimeout(120);
  const s = await stickyState();
  expect(s.rows[0]).toContain('workspace'); // 最外层 = 根标题行
  expect(s.rows[1]).toContain('归档'); // 其下是 归档compact
...
test('STICKY-5 回归（缓存不失效）：树上方的区增高但没 render → 滚深层仍吸对祖先', async () => {
  ...
  expect(s.rows[1]).toContain('归档');
  expect(s.rows[2]).toBe('华东区');
  expect(s.rows[3]).toBe('明细'); // 缓存 stale 会把这钉成别的行（本门变异自检过：还原绝对坐标即翻红）
});
