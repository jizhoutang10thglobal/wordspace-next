...
test('UX2: Cmd+W 关当前标签 / Cmd+T 弹模板台（菜单 onMenu 路由）', async () => {
  ...
  // Cmd+T → 'new-tab' → 弹 omni 模式新建 modal（browser UX 重做：顶部地址栏 + 模板台混排，无 .sb-modal-title）
  ...
  await expect(page.locator('.cm-omnibar-input')).toBeVisible();          // 顶部地址栏
  await expect(page.locator('.sb-card', { hasText: '空文档' })).toBeVisible(); // 模板台仍在
});
...
test('UI 防重叠：编辑器长文件名面包屑不压到右上角浮动 actions', async () => {
  ...
  const ta = await page.locator('.top-actions').boundingBox();
  // 限定文档头的面包屑——#web-header(网页头)里还有一条同类名的,严格模式会撞
  const bc = await page.locator('#doc-header .ws-breadcrumb').boundingBox();
