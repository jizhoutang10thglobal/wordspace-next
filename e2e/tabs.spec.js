...
// ===== viewer 态收编(0 根打开的文件 → 工作区出现后进标签系统;plan docs/plans/2026-07-17-001)=====
// 修前:0 根开文件走单文件 viewer 态(不建标签=设计),添加文件夹后文档游离在标签系统外、树点也救不回。

const menuAddFolder = () =>
  app.evaluate(({ Menu }) => {
    const walk = (items) => {
      for (const i of items) {
        if (i.label && /打开文件夹/.test(i.label)) { i.click(); return true; }
        if (i.submenu && walk(i.submenu.items)) return true;
      }
      return false;
    };
...
test('viewer 收编:0 根冷开文档 → 添加其文件夹 → 自动进标签(激活/无↗/不重载/去重/重启恢复)', async () => {
  await app.close();
  const dir = path.join(tmp, '贵阳六盘水');
  await fs.mkdir(dir, { recursive: true });
  const inside = path.join(dir, '龙脊山行.html');
  await fs.writeFile(inside, HTML('LJSX'), 'utf8');
  const ud2 = path.join(tmp, 'ud-viewer');
  ({ a: app, p: page } = await launch({ WS2_USERDATA: ud2, WS2_OPEN_FILE: inside, WS2_FOLDER_IN: dir }));
  await expect(page.frameLocator('#doc-frame').locator('h1')).toHaveText('LJSX');
  await expect(page.locator('#sidebar.sb-on')).toHaveCount(0); // 单文件 viewer 态:侧栏不挂(既有设计)
  // iframe 里塞 sentinel:收编若触发 reload 会丢(KD3 纯身份登记)
  await page.frameLocator('#doc-frame').locator('body').evaluate((b) => { b.__probe = 'alive'; });
  await menuAddFolder();
  const adopted = page.locator('#sb-tabs .sb-tab[data-rel="龙脊山行.html"]');
  await expect(adopted).toBeVisible(); // 修前:标签区「没有打开的标签」
  await expect(adopted).toHaveClass(/is-active/);
  await expect(adopted).not.toHaveClass(/sb-tab-ext/);
  await expect(page.locator('#sb-tabs .sb-tab')).toHaveCount(1);
  const probe = await page.frameLocator('#doc-frame').locator('body').evaluate((b) => b.__probe);
  expect(probe).toBe('alive'); // 没被 reload
  // 再触发一次树到货(添加第二根)→ 不翻倍
  await app.evaluate(({ }, d) => { process.env.WS2_FOLDER_IN = d; }, wsDir);
  await page.click('#sb-add-root');
  await expect(page.locator('.sb-root-head', { hasText: 'workspace' })).toBeVisible();
  await expect(page.locator('#sb-tabs .sb-tab')).toHaveCount(1);
  // 重启:收编身份已持久化
  await app.close();
  ({ a: app, p: page } = await launch({ WS2_USERDATA: ud2 }));
  await expect(page.locator('#sidebar.sb-on')).toBeVisible();
  const back = page.locator('#sb-tabs .sb-tab[data-rel="龙脊山行.html"]');
  await expect(back).toBeVisible();
  await expect(back).not.toHaveClass(/sb-tab-ext/);
});

test('viewer 收编:文件在新根外(warm 双击路径)→ ↗ 外部标签,点它仍是编辑器', async () => {
  await app.close();
  const outDir = path.join(tmp, 'outside');
  await fs.mkdir(outDir, { recursive: true });
  const outFile = path.join(outDir, 'solo.html');
  await fs.writeFile(outFile, HTML('SOLO'), 'utf8');
  ({ a: app, p: page } = await launch({ WS2_USERDATA: path.join(tmp, 'ud-warm'), WS2_FOLDER_IN: wsDir }));
  // warm「Finder 双击」:0 根 app 开着时来 open-file 事件
  await app.evaluate(({ app: a }, p) => { a.emit('open-file', { preventDefault() {} }, p); }, outFile);
  await expect(page.frameLocator('#doc-frame').locator('h1')).toHaveText('SOLO');
  await menuAddFolder(); // 添加的是别的文件夹(不含该文件)
  await expect(page.locator('#sidebar.sb-on')).toBeVisible();
  const ext = page.locator('#sb-tabs .sb-tab.sb-tab-ext');
  await expect(ext).toHaveCount(1); // 收编成 abs ↗ 外部标签(与「工作区在场时开根外文件」同款)
  await expect(ext).toHaveClass(/is-active/);
  await ext.click(); // kind 走 classifyFile:点击回编辑器,不是「默认应用打开」卡片
  await expect(page.frameLocator('#doc-frame').locator('h1')).toHaveText('SOLO');
  await expect(page.locator('#viewer')).toBeHidden();
});

test('viewer 收编:PDF 查看器文件也收(Colin 2026-07-17 拍板)', async () => {
  await app.close();
  const dir = path.join(tmp, 'pdf-dir');
  await fs.mkdir(dir, { recursive: true });
  const pdf = path.join(dir, '样张.pdf');
  await fs.writeFile(pdf, '%PDF-1.4\n1 0 obj<</Type/Catalog>>endobj\ntrailer<</Root 1 0 R>>\n%%EOF\n', 'utf8');
  ({ a: app, p: page } = await launch({ WS2_USERDATA: path.join(tmp, 'ud-pdf'), WS2_OPEN_FILE: pdf, WS2_FOLDER_IN: dir }));
  await expect(page.locator('#viewer')).toBeVisible(); // 查看器态(单文件模式)
  await menuAddFolder();
  const row = page.locator('#sb-tabs .sb-tab[data-rel="样张.pdf"]');
  await expect(row).toBeVisible(); // 修前:PDF 同样游离
  await expect(row).not.toHaveClass(/sb-tab-ext/);
  await expect(row).toHaveClass(/sb-kind-pdf/); // kind 正确 → 点击路由回查看器而非编辑器
});

test('KD3 web 门:web 激活时不收编;树点重入(U2 兜底);删掉不被树到货复活', async () => {
  await app.close();
  const dir = path.join(tmp, 'gate-dir');
  await fs.mkdir(dir, { recursive: true });
  const inside = path.join(dir, 'gate.html');
  await fs.writeFile(inside, HTML('GATE'), 'utf8');
  ({ a: app, p: page } = await launch({ WS2_USERDATA: path.join(tmp, 'ud-gate'), WS2_OPEN_FILE: inside, WS2_FOLDER_IN: dir }));
  await expect(page.frameLocator('#doc-frame').locator('h1')).toHaveText('GATE');
  // 立门桩:假装 web view 激活(真开网页标签要网络,桩位与真实判定同一 seam __webIsActive)
  await page.evaluate(() => { window.__wsRealWebActive = window.__webIsActive; window.__webIsActive = () => true; });
  await menuAddFolder();
  await expect(page.locator('.sb-file[data-rel="gate.html"]')).toBeVisible();
  await page.waitForTimeout(700); // 引擎是 async 尾随,给足窗口再断「没收编」
  await expect(page.locator('#sb-tabs .sb-tab')).toHaveCount(0); // KD3:web 激活 → 不收编、不抢激活
  // 撤桩,树里点它 → openDoc 同文档守卫的 U2 兜底补建
  await page.evaluate(() => { window.__webIsActive = window.__wsRealWebActive; });
  await page.click('.sb-file[data-rel="gate.html"]');
  const row = page.locator('#sb-tabs .sb-tab[data-rel="gate.html"]');
  await expect(row).toBeVisible(); // 修前(main@860eccf):点了也救不回,恒 0 条
  // 不复活:删 entry(测试 seam)+ 门重新立起 → 再来一次树到货 → 不重建
  await page.evaluate(() => { window.__webIsActive = () => true; });
  await page.evaluate((abs) => window.__sbHooks.dropEntryForTest(abs), inside);
  await expect(page.locator('#sb-tabs .sb-tab')).toHaveCount(0);
  await app.evaluate(({ }, d) => { process.env.WS2_FOLDER_IN = d; }, wsDir);
  await page.click('#sb-add-root');
  await expect(page.locator('.sb-root-head', { hasText: 'workspace' })).toBeVisible();
  await page.waitForTimeout(700);
  await expect(page.locator('#sb-tabs .sb-tab')).toHaveCount(0); // 门在:不复活
});
