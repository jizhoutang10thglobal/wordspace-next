...
test('快捷键 tooltip：核心操作控件 title 带快捷键提示（刷新/关标签/新建标签/另存为）', async () => {
  await openWorkspace();
  await page.click('.sb-file[data-rel="a.html"]');
  await expect(tabRow('a.html')).toBeVisible();
  // 标签行 × 关闭钮：⌘W（置顶区的 × 是「移出置顶」、无此提示，不在此验）
  await expect(tabRow('a.html').locator('.sb-tab-close')).toHaveAttribute('title', /Cmd\+W/);
  // 标签页区「+」新建标签：⌘T
  await expect(page.locator('#sb-tabs .sb-zone-add')).toHaveAttribute('title', /Cmd\+T/);
  // 导航条刷新：⌘R（DOM 里恒在，disabled 不影响 title）
  await expect(page.locator('#nav-reload')).toHaveAttribute('title', /Cmd\+R/);
  // ⋯ 菜单「另存为」：⌘⇧S
  await expect(page.locator('#save-btn')).toHaveAttribute('title', /Cmd\+Shift\+S/);
  // 既有提示未回退（回归兜底）：收起侧栏 ⌘\、查找文件 ⌘P
  await expect(page.locator('#sb-toggle')).toHaveAttribute('title', /Cmd\+\\/);
  await expect(page.locator('#sb-find')).toHaveAttribute('title', /Cmd\+P/);
});
