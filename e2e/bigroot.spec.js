...
// 不查内部 class 代理。
//
// ⚠ P0b 升级：原「过大」终态（.sb-root-oversize，不建树 + 内联移除）已被**简化模式（lazy 浏览）**取代——
//   超预算的根现在渲染可展开的 lazy 树 + 根行 .sb-root-lazy（照常显示路径，无可见徽标——Colin 2026-07-21）。U2/U5 已按新行为改写；
//   详见 e2e/bigroot-lazy.spec.js（P0b 全量验收 + lazy watcher/枚举降级）。
// ⚠ 变异自检（CLAUDE.md 铁律，血换）——**先 commit 再变异**，改回 `git checkout --` 会连修复一起冲掉：
//   打坏预算守卫（src/main/workspace.js walk：把 `if (count >= budget)` 改成 `if (false)`，或 treeBudget()
//   直接 return 1e9）→ 重跑本文件 → 「U2 简化模式」「U5 出口」两条必翻红（根不再进 lazy = 无 .sb-root-lazy）。
...
test('U2 过大→简化模式：超条目预算的根进 lazy（可浏览 + 徽标），可移除，app 全程可交互', async () => {
  const { p } = await launch({ WS2_FOLDER_IN: bigDir, WS2_TREE_BUDGET: String(BUDGET) });
  await p.click('#home-open-folder');
  // P0b：根进简化模式（lazy），不再是终态「过大」行。Colin 2026-07-21：lazy 根照常显示路径、不挂「Simplified」徽标。
  await expect(p.locator('.sb-root-lazy')).toBeVisible({ timeout: 8000 });
  await expect(p.locator('.sb-root-lazy .sb-root-path')).toContainText('海量文件夹');
  await expect(p.locator('.sb-root-lazy .sb-root-miss-tag')).toHaveCount(0);
  // app 可交互：树底「添加文件夹…」在且可点（没卡死）
  await expect(p.locator('#sb-add-root')).toBeVisible();
  // 右键根行「移除」→ 恢复正常空态（诊断 D4 红线：永远可移除）
  const rootId = (await p.$$eval('.sb-root-lazy', (els) => els.map((e) => e.dataset.root)))[0];
  await p.locator(`.sb-root-head[data-root="${rootId}"]`).click({ button: 'right' });
  await p.locator('.sb-ctx-item', { hasText: '移除' }).click();
  await expect(p.locator('.sb-root-lazy')).toHaveCount(0);
...
test('U5 拒绝出口：父根为简化模式时，加它的子文件夹给「去管理文件夹移除」出口（非空头支票）', async () => {
  // 先把 bigDir 作为简化模式（lazy）根打开
  const { p } = await launch({ WS2_FOLDER_IN: bigDir, WS2_TREE_BUDGET: String(BUDGET) });
  await p.click('#home-open-folder');
  await expect(p.locator('.sb-root-lazy')).toBeVisible({ timeout: 8000 });
  // 再加它的子文件夹 → 被判 child；父根 lazy → 新文案 + 「管理文件夹」按钮
