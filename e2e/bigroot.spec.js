...
//   超预算的根现在渲染可展开的 lazy 树 + 根行「简化模式」徽标（.sb-root-lazy）。U2/U5 已按新行为改写；
...
//   直接 return 1e9）→ 重跑本文件 → 「U2 简化模式」「U5 出口」两条必翻红（根不再进 lazy、无徽标）。
...
test('U2 过大→简化模式：超条目预算的根进 lazy（可浏览 + 徽标），可移除，app 全程可交互', async () => {
  ...
  // P0b：根进简化模式（lazy）——根行「简化模式」徽标真渲染，不再是终态「过大」行
  await expect(p.locator('.sb-root-lazy')).toBeVisible({ timeout: 8000 });
  await expect(p.locator('.sb-root-lazy .sb-root-miss-tag', { hasText: '简化模式' })).toBeVisible();
