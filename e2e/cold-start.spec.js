...
test('冷启动双击新文件，上次留有激活标签 → 新文件占 viewer + 旧标签仍恢复', async () => {
  ...
  // v2 多根格式：标签是全局单集合、entry 带 rootId、activeRel = rootId:rel。
  const storeFile = path.join(userData, 'workspace.json');
  const store = JSON.parse(await fs.readFile(storeFile, 'utf8'));
  const rootId = store.roots[0].id;
  store.tabs = {
    entries: [{ rootId, rel: 'a.html', kind: 'html', title: 'a.html', open: true, pinned: false }],
    activeRel: rootId + ':a.html',
  };
