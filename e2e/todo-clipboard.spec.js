...
test('U22 单行：外部纯文本单行「- [x] 单条」→ 1 项勾选 todo（clip-5 扩展）', async () => {
  await launch();
  await openDoc('<p id="p1">目标</p>');
  await frame.locator('#p1').click(); await page.keyboard.press('End'); await page.keyboard.press('Enter');
  await page.waitForTimeout(100);
  await setClipboardText('- [x] 单条任务');
  await page.keyboard.press('ControlOrMeta+v');
  await page.waitForTimeout(200);
  await expect.poll(() => frame.locator('ul.ws-todo > li').count(), { message: '单行 marker 也转 todo' }).toBe(1);
  expect(await frame.locator('ul.ws-todo > li').first().textContent()).toBe('单条任务');
  expect(await frame.locator('ul.ws-todo > li').first().getAttribute('data-checked')).toBe('true');
  expect(await conformOf(await serialize())).toBe(true);
});

test('U22 单行：「- [ ] 待办」→ 1 项未勾 todo', async () => {
  await launch();
  await openDoc('<p id="p1">目标</p>');
  await frame.locator('#p1').click(); await page.keyboard.press('End'); await page.keyboard.press('Enter');
  await page.waitForTimeout(100);
  await setClipboardText('- [ ] 待办一条');
  await page.keyboard.press('ControlOrMeta+v');
  await page.waitForTimeout(200);
  await expect.poll(() => frame.locator('ul.ws-todo > li').count()).toBe(1);
  expect(await frame.locator('ul.ws-todo > li').first().getAttribute('data-checked')).toBe(null);
  expect(await conformOf(await serialize())).toBe(true);
});

test('U22 单行负例：单行普通文字（无 marker）→ 仍字面、不转 todo', async () => {
  await launch();
  await openDoc('<p id="p1">目标</p>');
  await frame.locator('#p1').click(); await page.keyboard.press('End'); await page.keyboard.press('Enter');
  await page.waitForTimeout(100);
  await setClipboardText('随便一句话');
  await page.keyboard.press('ControlOrMeta+v');
  await page.waitForTimeout(150);
