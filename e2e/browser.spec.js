...
// 强断言纪律（S4）：view 是否真渲染用 win.capturePage 像素判（红底真上屏）,不只查 attach。
...
// 主进程侧取 web view 状态（registry 单例：require 缓存与 main 共享）。
function mainWebTabs(fn, arg) {
  return app.evaluate(({ app: a }, { fnSrc, arg }) => {
    const wt = require(require('path').join(a.getAppPath(), 'src/main/web-tabs.js'));
    // eslint-disable-next-line no-eval
    return eval('(' + fnSrc + ')')(wt, arg);
...
// 像素证据：win.capturePage 含子 view 合成结果,采样内容区中心像素（BGRA）。
async function centerPixel() {
  return app.evaluate(async ({ BrowserWindow }) => {
    ...
    const x = Math.floor(size.width * 0.65), y = Math.floor(size.height * 0.5);
...
test('⌘T 地址栏开网页：真加载(像素级红底上屏) + 标签行/omnibox/星标就位 + 无网页头', async () => {
  ...
  // 像素证据：内容区中心是 A 页的红底（view 真 attach + bounds 真铺满 + 真渲染;非白/非文档底）
  await expect.poll(async () => { const p = await centerPixel(); return p.r > 150 && p.g < 90 && p.b < 90; }, { timeout: 8000 }).toBe(true);
...
test('会话恢复：重启后 web 标签(url/title/pinned)回来,激活标签懒加载真渲染', async () => {
  ...
  await launch();
  await expect(page.locator('#sb-tabs .sb-tab.sb-tab-web .sb-name')).toHaveText('Page A', { timeout: 8000 });
  // 激活标签懒加载：恢复后 view 重建并真渲染（红底像素）
  await expect.poll(async () => { const p = await centerPixel(); return p.r > 150 && p.g < 90 && p.b < 90; }, { timeout: 10000 }).toBe(true);
  tmpDir = null;
});
...
test('右键菜单(原生 probe)：六分节按上下文/危险 scheme 整节滤/拷贝链接洗跟踪参数/动作白名单收口', async () => {
  ...
  const copied = await mainWebTabs((wt, { key, url }) => {
...
test('web↔doc 共存：切到文档标签摘 view(编辑器可见),切回网页 view 复原;omnibox 非网页回车开新网页标签', async () => {
  ...
  await expect.poll(async () => { const p = await centerPixel(); return p.r > 150 && p.g < 90; }, { timeout: 8000 }).toBe(true); // 网页盖上来了
  // 切回文档标签 → view 摘掉,编辑器像素回白底（红色消失）
  await page.locator('#sb-tabs .sb-tab:not(.sb-tab-web)').click();
  await expect.poll(async () => { const p = await centerPixel(); return p.r > 150 && p.g < 90; }, { timeout: 8000 }).toBe(false);
  await expect(page.frameLocator('#doc-frame').locator('h1')).toHaveText('AAA');
  // 切回网页标签 → view 复原（红底回来）
  await page.locator('#sb-tabs .sb-tab.sb-tab-web').click();
  await expect.poll(async () => { const p = await centerPixel(); return p.r > 150 && p.g < 90; }, { timeout: 8000 }).toBe(true);
});
