...
test('U0：文档内 http 外链 → 走系统程序（openExternalUrl），iframe 不导航', async () => {
  ...
  await expect.poll(() => app.evaluate(() => globalThis.__extCalls || []), { timeout: 3000 }).toContain('https://example.com/'); // U3-B1：poll 外链调用,替死等 300ms
  // iframe 没被导航走：还停在 C
  await expect(frame.locator('h1')).toHaveText('文档C');
...
test('U3：@ 提及 → 选文档 → 插入纯净 <a href>（磁盘零 class/contenteditable/&nbsp;）', async () => {
  ...
  await expect.poll(() => fs.readFile(path.join(wsDir, 'D.html'), 'utf8'), { timeout: 5000 }).toMatch(/<a href="B\.html">文档B<\/a>/); // U3-B1：poll 落盘,替死等 1700ms(慢机零 flake)
...
test('U3-B6：从侧栏拖文件进正文 → 插入纯净链接（真实拖拽管线）', async () => {
  ...
  await expect.poll(() => fs.readFile(path.join(wsDir, 'D.html'), 'utf8'), { timeout: 5000 }).toMatch(/<a href="B\.html">/); // U3-B1：poll 落盘,替死等 1700ms
  const d = await fs.readFile(path.join(wsDir, 'D.html'), 'utf8');
...
test('U3-B5：选中文字 → 气泡「链接」→ 选文档 → 选中文字变链接（wrap 保留文字）', async () => {
  ...
  await expect.poll(() => fs.readFile(path.join(wsDir, 'D.html'), 'utf8'), { timeout: 5000 }).toMatch(/<a href="B\.html">草稿<\/a>/); // U3-B1：poll 落盘,替死等 1700ms
  const d = await fs.readFile(path.join(wsDir, 'D.html'), 'utf8');
...
test('U3-B2：@菜单也列非文档文件（pdf）', async () => {
  ...
  await page.keyboard.press('Enter');
  await expect.poll(() => fs.readFile(path.join(wsDir, 'D.html'), 'utf8'), { timeout: 5000 }).toMatch(/<a href="[^"]*报告[^"]*\.pdf">报告<\/a>/); // U3-B1：poll 落盘,替死等 1700ms
  const d = await fs.readFile(path.join(wsDir, 'D.html'), 'utf8');
...
test('U4：修复卡「重新指向」→ 改写 href 到候选（保留 #尾缀）+ 自愈', async () => {
  ...
  // 自愈：高亮清零
  await expect.poll(brokenSize, { timeout: 5000 }).toBe(0);
  // 落盘：C.html 的 href 真被改（poll 到落盘为止,替死等 1.2s+,U3-B1）
  await expect.poll(() => fs.readFile(path.join(wsDir, 'C.html'), 'utf8'), { timeout: 5000 }).toContain('归档/缺失.html#节2');
  const c = await fs.readFile(path.join(wsDir, 'C.html'), 'utf8');
...
test('U7：html 主动保存补 wordspace-doc-id（缺失才补、复用不换、单个、不破合规）', async () => {
  ...
  await expect.poll(() => fs.readFile(path.join(wsDir, 'D.html'), 'utf8'), { timeout: 5000 }).toMatch(/wordspace-doc-id"\s+content="[^"]+"/); // U3-B1：poll 落盘 doc-id,替死等 1700ms
...
test('U7：外部改名后靠 doc-id → 修复卡「原文件已移动到这里」置顶候选 + 重新指向', async () => {
  // 给 A/B 加 doc-id（外部改名要靠它认出文件搬去哪）；A 链 B
  await fs.writeFile(path.join(wsDir, 'A.html'), '<!doctype html><html><head><meta charset="utf-8"><meta name="wordspace-doc-id" content="ID-A"></head><body><h1>文档A</h1><p>去 <a href="B.html">B</a></p></body></html>', 'utf8');
  await fs.writeFile(path.join(wsDir, 'B.html'), '<!doctype html><html><head><meta charset="utf-8"><meta name="wordspace-doc-id" content="ID-B"></head><body><h1>文档B</h1><p>b</p></body></html>', 'utf8');
  await page.click('.sb-file[data-rel="A.html"]'); // 打开 A → 建索引 + 快照 A→B 的 targetDocId=ID-B
  const frame = page.frameLocator('#doc-frame');
  await expect(frame.locator('h1')).toHaveText('文档A');
  const rootId = await page.evaluate(async () => (await window.ws2.wsGetRoots())[0].id);
  // 外部改名 B.html → 挪走了.html（doc-id 随字节不变）
  await fs.rename(path.join(wsDir, 'B.html'), path.join(wsDir, '挪走了.html'));
  // 等 watcher 刷索引 + carry-forward：movedTarget 反查到新址（索引就绪的确定信号）
  await expect.poll(() => page.evaluate((id) => window.ws2.linksMovedTarget(id, 'A.html', 'B.html'), rootId), { timeout: 8000 }).toBe('挪走了.html');
  // 点 A 里那条现已断的 B 链接（点击时 resolveDocLink 现查、权威）→ 弹修复卡
  await frame.locator('a[href="B.html"]').click();
  await expect(page.locator('.ws-linkview-card.is-broken')).toBeVisible();
  // 置顶候选 = doc-id 反查到的现址
  const moved = page.locator('.ws-linkview-repair-item', { hasText: '原文件已移动到这里' });
  await expect(moved).toContainText('挪走了.html');
  await moved.click();
  // href 重写成新址、断链自愈
...
test('U5：改名时打开中的链接文档 → 内存改 href + toast + 落盘（不和自动保存打架）', async () => {
  ...
  // 落盘：等自动保存，A.html disk 也变（poll 到落盘为止,U3-B1）
  await expect.poll(() => fs.readFile(path.join(wsDir, 'A.html'), 'utf8'), { timeout: 5000 }).toContain('BB.html');
  const a = await fs.readFile(path.join(wsDir, 'A.html'), 'utf8');
