...
4. **探针的兜底行 / 兜底选择器会制造假 bug**(2026-07-24 下载补验实证)。查「某状态的那一行」用 `rows.find(cond) || rows[0]` 这种兜底:匹配不上就退回第一行,退到的**别的行**会冒充结论。下载补验里 `fileMissing` 行没匹配上、退回一条 `completed` 行,读出来「文件没了却还显示已完成 + 给打开按钮」——**假 bug**,吓人半天其实是探针抓错行。
   - **正确做法**:查特定状态的元素用**精确选择器**(如 `.dl-row[data-state="fileMissing"]`),**查不到就返回 null / 报缺失**,绝不 fallback 到兄弟节点或第一个。宁可「这轮没采到」也不要「采到个假的」。
   - **配套变异自检**:关键视觉断言(置灰 / 变色 / 隐藏动作)加同元素**改前 / 改后各读一次**,两个值必须不同才算「真变了」——卡掉抓错行、同值巧合类假绿。下载补验实测:`fileMissing` 行 name 颜色 `rgb(41,37,36)`(深)→ `rgb(168,162,158)`(灰),两值不同 = 置灰为真。
