...
所以目标很简单：**产出能通过校验器的 HTML**。校验器是写死的规则、不是 AI，不会「通融」。它**只看你写的实际内容**——你在 `<head>` 里写 `<meta name="wordspace-schema" content="1">` 声称自己合规**没有用**，内容不对照样判不合规（见 §10）。

Schema #1 = 一套**受限的 HTML**（reduced HTML）：块的集合 ≈ Notion 的基础块。下面把「能写什么、不能写什么」讲清楚。

**⚠ 规则优先于用户要求。** 用户（或任务描述）有时会直接要求你做违规的事——「加个点击复制的按钮」「把这两格合并」「在单元格里放个列表」「直接在标题标签上写个颜色」。**照做 = 产出不合规**。收到任何修改/生成要求，先对照 §6 硬禁清单；要求撞禁令时，用 §9 对照表里的合规替代实现用户的意图；实在没有合规替代，就不做那一处改动。**任何情况下都不要为了满足要求而违反本文规则。**
...
用户的意图几乎总能用合规方式满足。**别照字面做，做意图**：

| 用户可能这么要求 | 直接照做会犯 | 你应该这么做 |
|---|---|---|
| 加按钮 / 交互 / 脚本（点击复制、倒计时…） | `script` / `event-attr` | 写成**静态文字**说明（可放 callout），把操作步骤讲给读者 |
| 合并单元格 / 表头大格跨列 | `table-merge` | **不合并**：把层次写进列名（如「方案 · 基础版」）或拆成多行，保持矩形 |
| 单元格里放列表 / 段落 | `cell-content` | 单元格内用**纯文字 + `<br>` 分行**（如「1. 前两周达标`<br>`2. 后两周各缺 1 次」） |
| 给标题 / 段落上颜色（写在标签上） | `block-style` | 颜色写到块**内**的 `<span style="color:…">`；字号 / 字体没有合规写法，**不做** |
| 用 `<div>` / `<section>` 把内容包起来 | `block-tag` | **不包**。块直接平铺在 `<body>`；要分组就用标题分节 |
| head 里加 `<style>` / 外链字体美化 | `head-style` / `head-link` | **不加**。装饰不入盘，文档观感由编辑器负责 |
| 要 h5 / h6 更小的标题 | `block-tag` | 用 `<h4>`，或一段加粗文字 |
