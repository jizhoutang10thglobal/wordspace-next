...
| Schema | 范式 | 规范 | 怎么认 |
|---|---|---|---|
| **#1** | 类 Notion 文档流（标题/段落/列表/表格/折叠块），不分页 | `references/schema-1.md` | `<head>` 里**没有** `<style data-ws-schema-css="page">`（`meta wordspace-schema content="1"` 是提示） |
| **#2** | 分页文档（Schema #1 的全部规则 + 按页排版，像 Word：纸张/边距/页眉页脚） | `references/schema-2.md` | `<head>` 里有可解析的 `<style data-ws-schema-css="page">@page{…}</style>`（`meta wordspace-schema content="2"` 是提示） |

- **生成新文档** → 用户要「像 Word 那样分页 / 有纸张 / 页眉页脚」→ Schema #2；否则默认 Schema #1（流式）。
- **编辑已有文档** → 看 `<head>` 有没有可解析的 page 块：有 = Schema #2，按 `references/schema-2.md`；没有 = Schema #1。（**判定只认内容不看 `wordspace-schema` meta**；meta 只帮你快速选规范，认错了校验器/归类器也兜底。）
...
`node scripts/validate-schema.js <file.html|file.md>`（退出码 0 = 合规，stdout 给 JSON violations）。
