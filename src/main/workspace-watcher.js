...
// 同 rootId 重复 watch（重新定位换路径）会先关旧的。
function watch(rootId, rootPath, onChange) {
  unwatch(rootId);
  ...
  try {
    ...
    // 根被外部删除/改名时 Windows 会发 EPERM error → 静默放弃该根的监听，靠聚焦兜底。
    entry.watcher.on('error', () => unwatch(rootId));
