...
(function () {
  ...
  async function handleAddResult(r) {
    ...
    if (r.status === 'same') {
      ...
      if (pst && (pst.loading || pst.truncated)) {
        showToast(window.wsT('sidebar.folderChildParentStuck', { name: r.name, parent: r.parent.name }), window.wsT('menu.manageRoots'), () => openManageRootsModal());
      } else {
  ...
  function renderOversizeRoot(root, index, parent = treeEl) {
    ...
    head.dataset.rel = '';
    head.dataset.depth = -1;
    head.title = window.wsT('sidebar.oversizeHeadTitle', { path: root.path });
    const ico = document.createElement('span');
    ico.className = 'sb-ico sb-root-miss-ic';
    ico.innerHTML = WARN_SVG20;
    const name = document.createElement('span');
    ...
    const tag = document.createElement('span');
    tag.className = 'sb-root-miss-tag';
    tag.textContent = window.wsT('sidebar.oversizeTag');
    head.append(ico, name, tag);
    head.oncontextmenu = (e) => {
      e.preventDefault();
      const items = [];
    ...
    const msg = document.createElement('span');
    msg.className = 'ws-truncate';
    msg.textContent = window.wsT('sidebar.oversizeNote');
  ...
  if (reopenBtn) reopenBtn.onclick = () => { setSidebarCollapsed(false); coachOnce('toggle-sidebar', window.wsT('sidebar.coachToggleSidebar', { key: kbd('⌘\\') })); };
