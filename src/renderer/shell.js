...
function runUndoRedo(isRedo) {
  if (!undoMgr) return;
  const editSnap = blockEdit && blockEdit.snapshotEdit ? blockEdit.snapshotEdit() : null; // U8/clip-3：重写前记录编辑块（id + 结构路径）
  const changed = isRedo ? undoMgr.redo() : undoMgr.undo();
  if (!changed) return;
  if (blockEdit) { blockEdit.reset(); if (blockEdit.restoreEdit) blockEdit.restoreEdit(editSnap); } // U8：重写后按 id/路径重进编辑，别让焦点落非可编辑 body → 打字被吞
  if (basicEdit) {
    basicEdit.detach();
